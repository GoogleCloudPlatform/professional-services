...
class Chat extends Component {
    ...
    renderActiveChat() {
        if (!this.props.state || !this.props.state.chatList || !this.props.state.activeUser) {
            return (<div><h3>No active chat</h3></div>)
        }
        var activeUser = this.props.state.activeUser
        var activeChat = this.props.state.chatList.filter(chat => chat.user === activeUser)[0]
        if (activeChat) {
            return (<div>
                ...
                <form onSubmit={this.handleSubmit} className="col align-self-end">
                    <FormGroup
