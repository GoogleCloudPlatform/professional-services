...
const api = store => next => action => {
    ...
    switch (action.type) {
        case constants.LOGIN:
            var loginUrl = store.getState().login.url + 'chat/login/' + store.getState().login.user + '/' + store.getState().login.language
            request
                .post(loginUrl)
                .end((err, res) => {
                    if (err) {
        ...
        case constants.POLL_SERVER:
            if (!store.getState().polling) {
                var getUrl = store.getState().login.url + 'chat/message/' + store.getState().login.user + '/' + store.getState().login.language
                request
                    .get(getUrl)
                    .end((err, res) => {
                        if (err) {
        ...
        case constants.SEND_MSG:
            var sendMsgUrl = store.getState().login.url + 'chat/message'
            request
        ...
        case constants.ADD_CHAT:
            var chatHistoryUrl = store.getState().login.url + 'chat/history/' +
