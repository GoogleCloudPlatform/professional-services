...
public class SpannerService {
    ...
    public void initDb() {
        ...
        Iterator<Database> databaseIterator = dbAdminClient.listDatabases(instanceId, Options.pageSize(10)).iterateAll().iterator();
    ...
    public List<Message> getMessages(String recipient) {
        ...
        try (ResultSet rs =
                     dbClient
                             .singleUse()
                             .executeQuery(Statement.newBuilder("SELECT * FROM messages WHERE " +
                                     "recipient = @recipient AND " +
                                     "read = FALSE ORDER BY create_time ASC;")
                                     .bind("recipient").to(recipient)
                                     .build())) {
            while (rs.next()) {
        ...
        for (Message message : messages){
            mutations.add(Mutation.newUpdateBuilder(TABLE).set(ID_ATTRIBUTE)
                    .to(message.id)
    ...
    private List<Mutation> buildMutations(Message... m) {
        return  Arrays.asList(m).stream().map(message -> Mutation.newInsertBuilder(TABLE)
                .set(ID_ATTRIBUTE)
                .to(message.id)
                .set(RECIPIENT_ATTRIBUTE)
                .to(message.recipient)
    ...
    public String queryTest() {
        ResultSet resultSet =
                dbClient
                        .singleUse()
                        .executeQuery(Statement.of("SELECT ID_ATTRIBUTE, sender, message FROM messages LIMIT 2"));
