...
from gsuite_exporter.fetcher import AdminAPIFetcher
...
def sync_all(
    ...
    Args:
        ...
        app (str): The Gsuite Admin Application to query.
        project_id (str): The project id to export the data to.
        exporter (str): The exporter class to use.
    """
    fetcher = AdminAPIFetcher(credentials_path, token_path, api)

    exporter = get_exporter_cls(exporter)(credentials_path, project_id, app)
    ...
    for records in records_stream:
        ...
        print("Last timestamp after export: {}".format(exporter.last_timestamp))
...
def main():
    ...
    parser.add_argument('--app', type=str, help='The GSuite Admin Application', required=True)
