...
def sync_gcp_service_routes(project, routes_prefix, priority, network, next_hop_gateway,
                            apply_changes=False,
    ...
    all_project_routes = compute_service.list_routes(project)
    ...
    if apply_changes:
        for route in add_routes:
            _route = compute_service.insert_default_internet_gateway_route(project=project, name=routes_prefix + str(uuid1()), priority=priority,
    ...
    if apply_changes:
        for route in delete_routes:
            ...
            for route_name in current_routes_maps[route]:
                ...
                compute_service.delete_route(project=project,route=route_name)
    else:
