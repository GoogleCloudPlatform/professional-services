...
get_zone $l
gcloud beta container clusters create slots-deficit-cluster --project $p --region $zone --machine-type "n1-standard-2" --num-nodes "1" --image-type "COS" --service-account "slots-deficits-log-service@$p.iam.gserviceaccount.com" --enable-cloud-logging --enable-cloud-monitoring --enable-autoupgrade --enable-autorepair
