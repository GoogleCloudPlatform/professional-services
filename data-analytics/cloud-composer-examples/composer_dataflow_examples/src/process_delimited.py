...
class ParseDelimited(object):
    ...
    def __init__(self, delimiter, header, filename, load_dt):
        ...
        self.filename_dict = {'filename': filename}
        self.load_dt_dict = {'load_dt': load_dt}
    ...
    def parse(self, row):
        ...
        values = re.split(self.delimiter, re.sub('\r\n', '', re.sub(u'"', '', row)))

        row = dict(zip(self.keys, values))

        # Add an additional filename field.
        row_with_filename = merge_two_dicts(
            row, self.filename_dict)

        # Add an additional load_dt field.
        row_with_filename_load_dt = merge_two_dicts(
            row_with_filename, self.load_dt_dict)

        return row_with_filename_load_dt


def merge_two_dicts(first_dict, second_dict):
    """A utility function to merge two dictionaries."""
...
def run(argv=None):
    ...
    parse_delimited = ParseDelimited(delimiter=known_args.delimiter,
                                     header=known_args.fields,
                                     filename=ntpath.basename(known_args.input),
                                     load_dt=known_args.load_dt)

    # We use the save_main_session option because the workflow relies
    # on a function in the global context.
    p_opts = pipeline_options.PipelineOptions(pipeline_args)
    p_opts.view_as(pipeline_options.SetupOptions).save_main_session = True
    ...
    with beam.Pipeline(options=p_opts) as pipeline:
        ...
        dict_records = rows | "Convert to BigQuery row" >> beam.Map(
            lambda r: parse_delimited.parse(r))
