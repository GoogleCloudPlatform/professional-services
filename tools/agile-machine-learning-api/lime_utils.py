...
def download_file_from_gcp(bucket_name, source_blob_name):
    ...
    try:
        ...
        raise AssertionError("ERROR: Kindly verify the access to GCP bucket")
...
def getting_signature(export_dir):
    ...
    col_list = [
        a.name for a in graph.as_graph_def().node if a.op == 'Placeholder'
    ]
    inputs = dict()
    for t in col_list:
    ...
    output_name = [
        n.name for n in graph.as_graph_def().node if n.op == 'Softmax'
    ][0]
    output = tf.saved_model.build_tensor_info(
        graph.get_tensor_by_name(str(output_name) + ':0'))
    signature = tf.saved_model.build_signature_def(inputs=inputs,
...
def get_explainer(export_dir):
    ...
    """
    with open(os.path.join(export_dir, 'assets.extra', 'lime_explainer'),
              'rb') as f:
...
def lime_predictor(explainer, dict_mapping, feature_names, data_point,
                   predictor):
    """
        ...
        for index, feature_name in enumerate(feature_names):
            if feature_name in dict_mapping.keys():
                ...
                for cat_value in X[:, index]:
                    feat_list.append(dict_mapping[feature_name][int(cat_value)])
    ...
    exp = explainer.explain_instance(np.array(feat_list),
                                     predict_function,
                                     num_features=5)
    return exp
...
def adding_text_to_html(html_file, explanation, dict_mapping):
    ...
    for (feature, importance) in explanation.as_list():
        if feature.split('=')[0] in dict_mapping.keys():
            text_exp += 'Categorical feature "{}" being "{}" is contributing to the prediction with importance of \
                {} <br>'.format(
                feature.split('=')[0],
                feature.split('=')[1], "{0:.3f}".format(importance))
            text_exp += ' \n'
        else:
    ...
    string = '''<div style="text-align:right;"><a href="#" onclick="document.getElementById('text-explanation').classList.toggle('is-hidden')">info</a></div>
            ...
            <div id="text-explanation" style="text-align:right;" class="is-hidden">''' + str(
        text_exp) + ''' </div>'''
...
def visualization(cfg, job_id, model_dir, predict_json, batch_prediction,
                  d_points, name):
    ...
    if batch_prediction is False:
        explanation = lime_predictor(explainer=explainer,
                                     dict_mapping=dict_mapping,
                                     feature_names=feature_names,
                                     data_point=predict_point,
                                     predictor=predictor)
        html_file = explanation.as_html()
        save(gcp_bucket_name,
        ...
        for key, value in predict_point.items():
            for extra_key in [
                    a for a in value.keys() if a not in feature_names
            ]:
                del value[extra_key]
...
def visualization_2(cfg, job_id, model_dir, predict_json, batch_prediction,
                    name):
    ...
    if batch_prediction is False:
        ...
        for index, prediction_point in enumerate(predict_point):
            ...
            for extra_key in [
                    a for a in prediction_point.keys() if a not in feature_names
            ]:
                del prediction_point_copy[extra_key]
            result.append(
                prediction_without_report(prediction_point_copy, predictor))
            if prediction_point['report'] == 1:
                for extra_key in [
                        a for a in prediction_point.keys()
                        if a not in feature_names
                ]:
                    del prediction_point[extra_key]
                explanation = lime_predictor(explainer=explainer,
                                             dict_mapping=dict_mapping,
                                             feature_names=feature_names,
                                             data_point=prediction_point,
                                             predictor=predictor)
                html_file = explanation.as_html()
