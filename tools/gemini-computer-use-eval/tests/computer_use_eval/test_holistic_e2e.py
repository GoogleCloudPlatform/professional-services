...
def mock_judges():
    ...
    with patch("computer_use_eval.runner.AssertionJudge") as MockDetJudge, \
        ...
        MockVideoJudge.return_value.evaluate = AsyncMock(
            return_value={"score": 1.0, "reasoning": "Holistic success"}
        )
...
class TestHolisticE2E:
    @pytest.mark.asyncio
    async def test_runner_multi_video_integration(
        self, mock_env, mock_agent, mock_judges
    ):
        """
        ...
        config = {
            ...
            "task": {"goal": "Search in Tab 1, Click in Tab 2"},
    ...
    async def test_action_registry_holistic(self):
        ...
        assert isinstance(
            executor.actions[ToolNames.DOUBLE_CLICK_AT], DoubleClickAction
        )
    ...
    async def test_judge_signature_consistency(self, mock_env):
        ...
        with patch.object(
            logj, "_evaluate_custom", new_callable=AsyncMock
        ) as mock_eval:
            ...
            # Should not raise TypeError
            await logj.evaluate(mock_env, criteria, history=[], video_paths=v_paths)

        # Test VideoJudge
        vidj = VideoJudge(MagicMock())
        with patch.object(vidj, "evaluate", new_callable=AsyncMock) as mock_eval:
            await vidj.evaluate(mock_env, criteria, video_paths=v_paths)
            mock_eval.assert_called_once_with(mock_env, criteria, video_paths=v_paths)
