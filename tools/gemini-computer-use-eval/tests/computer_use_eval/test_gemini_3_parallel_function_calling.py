...
async def test_parallel_function_responses_single_image_attachment():
    ...
    agent._execute_batch = AsyncMock(
        return_value=(
            [
    ...
    real_history = []
    agent.history_manager.add_content = MagicMock(side_effect=real_history.append)
    ...
    assert (
        user_turns[0].parts[0].function_response.parts is None
        or len(user_turns[0].parts[0].function_response.parts) == 0
    )

    assert user_turns[1].parts[0].function_response.id == "id_2"
    assert (
        user_turns[1].parts[0].function_response.parts is None
        or len(user_turns[1].parts[0].function_response.parts) == 0
    )
    ...
    assert (
        user_turns[2].parts[0].function_response.parts[0].inline_data.mime_type
        == "image/png"
    )
...
async def test_image_context_strategy_preserves_function_response_ids():
    ...
    strategy = ImageContextStrategy(
        max_images=1, retention_strategy=ImageRetentionStrategy.VARIABLE_FIDELITY
    )
    strategy._degrade_image = MagicMock(return_value=b"downgraded_bytes")

    fr_part1 = types.FunctionResponsePart(
        inline_data=types.FunctionResponseBlob(
            mime_type="image/png", data=b"original_bytes_old"
        )
    )
    old_fr = types.FunctionResponse(
        name="click_at", id="vital_id_123", response={"status": "ok"}, parts=[fr_part1]
    )

    fr_part2 = types.FunctionResponsePart(
        inline_data=types.FunctionResponseBlob(
            mime_type="image/png", data=b"original_bytes_new"
        )
    )
    new_fr = types.FunctionResponse(
        name="click_at", id="vital_id_456", response={"status": "ok"}, parts=[fr_part2]
    )

    history = [
        types.Content(role="user", parts=[types.Part(text="start")]),
        types.Content(role="user", parts=[types.Part(function_response=old_fr)]),
        types.Content(role="model", parts=[types.Part(text="thinking")]),
        types.Content(role="user", parts=[types.Part(function_response=new_fr)]),
    ...
    mock_predict_response.candidates = [
        MagicMock(
            content=types.Content(
                ...
                parts=[
                    types.Part(
                        function_call=types.FunctionCall(name="type_text_at", args={})
                    )
                ],
            )
        )
