...
def check_missing(attribute_name: str,
                  analysis: run_metadata_pb2.Analysis) -> Union[None, str]:
    ...
    if missing_rate > MISSING_THRESHOLD:
        return report_template.HIGH_MISSING.format(
            name=attribute_name,
            value=missing_rate
        )
    else:
...
def check_cardinality(attribute_name: str,
    ...
    if cardinality > CARDINALITY_THRESHOLD:
        return report_template.HIGH_CARDINALITY.format(
            name=attribute_name,
            value=cardinality
        )
    else:
...
def check_pearson_correlation(analysis: run_metadata_pb2.Analysis
    ...
    if abs(coefficient) > CORRELATION_COEFFICIENT_THRESHOLD:
        return report_template.HIGH_CORRELATION.format(
...
def check_p_value(analysis: run_metadata_pb2.Analysis
    ...
    if p_value < P_VALUE_THRESHOLD:
        return report_template.LOW_P_VALUE.format(
            name_one=name_list[0],
            name_two=name_list[1],
            metric='p-value',
