...
def create_table_descriptive_row_from_analysis(
    ...
    """Create makrdown formated descriptive analysis result
    ...
    """
    row_template = report_template.TABLE_DESCRIPTIVE_ROW_TEMPLATE
    stats_template = report_template.TABLE_DESCRIPTIVE_STATS_TEMPLATE
...
def create_table_from_TableMetric(
    ...
    table_template = report_template.TABLE_TEMPLATE
    ...
    for row in table_metric.rows:
        ...
        row_header = report_template.BOLD.format(
...
def create_pairwise_metric_table(row_list: Set[str],
    ...
    for row_name in row_list:
        # row header is in BOLD
        row_values = [report_template.BOLD.format(content=row_name.strip())]
...
def create_no_order_pair_metric_section(
    ...
    if table_name != "NA":
        ...
        figure_content = report_template.IMAGE_TEMPLATE.format(
            url=figure_path,
            alt_text=table_name
        )
    else:
...
def create_target_metrics_highlight(
        ...
        metric_analysis_list: List[List[run_metadata_pb2.Analysis]]) -> str:
    assert len(metric_name_list) == len(metric_analysis_list)
    # Every metric should have the same length, i.e., target v.s. remainings
    assert len(set([len(item) for item in metric_analysis_list])) == 1
    ...
    table_template = report_template.TARGET_METRIC_HIGHLIGHT_TEMPLATE
    row_template = report_template.TARGET_METRIC_HIGHLIGHT_ROW_TEMPLATE
...
def create_content_list(contents: List[str]) -> str:
    return '\n'.join(
        [report_template.LIST_TEMPLATE.format(
            level='',
            content=item
...
def create_warning_notes(warnings: List[str]) -> str:
    warning_title = report_template.SUB_SUB_SECTION_TITLE.format(
        content='Warnings'
    )
