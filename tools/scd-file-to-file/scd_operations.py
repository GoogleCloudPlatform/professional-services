...
def scd_update_delete_generation(
    ...
    try:
        ...
        df = pd.read_csv(gcs_input_path, storage_options={'client': storage_client}, header=0)

        scd_staging_temp = df[df[active_flag_column] == True].copy() # Create a copy to avoid SettingWithCopyWarning
        ...
        print(f"Update SCD Count Generation={total_records}, Unique SCD Keys for Generation={unique_scd_keys_for_generation}")
        columns_to_process = scd_column_list.split(",")

        for current_column in columns_to_process:
            # Get unique values for SCD generation
            scd_array = df[current_column].drop_duplicates().head(unique_scd_keys_for_generation).values
            ...
            batch_size = max(int(total_records / array_len), 1) # Ensure batch_size is at least 1
            print("Batch Size:", batch_size) # Print the batch size for debugging
            i = 1
            array_index = 0
            unporcessed_record_count = len(scd_staging[scd_staging['processed_flag'] == False])

            while unporcessed_record_count > 0: # Loop while there are unprocessed records
                j = i
                array_value = scd_array[array_index % array_len]
                k = min(i + batch_size - 1, total_records)

                mask = (
                    (scd_staging[current_column] != array_value) &
                    (scd_staging['processed_flag'] == False)
                )
            ...
            scd_staging['processed_flag'] = False # Reset processed flag for next column


        p_status = "SCD Update Delete Generation succeeded"

        #scd_staging = scd_staging.sort_values(by=[primary_key_column])
        scd_staging = scd_staging.drop(columns=['id','effective_from_date', 'effective_to_date', 'active_flag', 'processed_flag'])
        ...
        scd_staging_delete = scd_staging_delete.drop(columns=['effective_from_date', 'effective_to_date', 'active_flag'])
...
def scd_insert_generation(
    ...
    try:
        ...
        print(f"Insert SCD Count Generation={total_records}, Unique SCD Keys for Generation={unique_scd_keys_for_generation}")
        ...
        scd_staging = scd_staging.sort_values(by=[primary_key_column])
        scd_staging = scd_staging.drop(columns=['id','effective_from_date', 'effective_to_date', 'active_flag', 'processed_flag'])
        scd_staging[primary_key_column] = scd_staging.apply(lambda _: uuid.uuid4().int, axis=1)
        ...
        # Upload the DataFrame to GCS
        blob.upload_from_string(scd_staging.to_csv(index=False), content_type='text/csv')

        print(f"DataFrame uploaded to: gs://{gcs_bucket}/{gcs_file_path}")

    except Exception as e:
        p_status = f"SCD Insert Generation Failed: {e}" # Include the actual error message
