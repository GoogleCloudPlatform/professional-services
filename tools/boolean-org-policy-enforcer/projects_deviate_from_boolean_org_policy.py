...
def is_org_policy_enforced(project_id, constraint, credentials):
...
def filter_projects_having_org_policy_not_enforced(project_ids, constraint,
  ...
  """
  f = functools.partial(
      is_org_policy_enforced, constraint=constraint, credentials=credentials)
  ...
  while i < n:
    ...
    with futures.ThreadPoolExecutor(max_workers=max_request) as executor:
      org_policy_enforced_state = executor.map(f,
                                               project_ids[i:i + max_request])
    i += max_request
...
if __name__ == "__main__":
  ...
  print("Total number of projects in your organization is " +
        str(len(all_project_ids)) + ".")
  print("It would take approximately ",
        len(all_project_ids) / RATE_LIMIT[0] * RATE_LIMIT[1],
        " seconds to finish this script.")
  impacted_projects = filter_projects_having_org_policy_not_enforced(
      all_project_ids, args.boolean_constraint, args.constraint_expected_state,
      args.organization, sa_credentials)

  if not args.to_json:
    print(impacted_projects)
  else:
    to_json(impacted_projects, args.to_json)
