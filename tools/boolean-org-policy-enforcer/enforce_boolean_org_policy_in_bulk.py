...
def enforced_org_policy_for_a_project(project_id, constraint, expected_state,
                                      credentials):
  ...
  new_policy = resourcemanager_v1.projects().setOrgPolicy(
      resource="projects/" + project_id, body=enforced_policy_body).execute()
...
def enforce_org_policy(data_location, credentials):
  ...
  print(
      "Total number of projects in your organization with org-policy not "
      "enforced is " + str(len(project_ids)) + ".")
  print("It would take approximately ",
        len(project_ids) / RATE_LIMIT[0] * RATE_LIMIT[1],
  ...
  f = functools.partial(
      enforced_org_policy_for_a_project,
      constraint=constraint,
...
if __name__ == "__main__":
  ...
  if not args.to_json:
    ...
    to_json(new_org_policies, args.to_json)
