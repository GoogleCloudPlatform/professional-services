...
class MonolithicTestCase(unittest.TestCase):
  ...
  def step03_support_create_case(self):
    ...
    support_create_case_output = support_create_case(
        ...
        description, severity, classification_id, classification_display_name,
        time_zone, self.project_number, test_case)
  ...
  def step04_firestore_write(self):
    ...
    """
    self.resource_name = f"projects/{self.project_number}/cases/{self.case}"
    content = {
        "case_number":
            self.case,
        "resource_name":
  ...
  def step20_support_add_comment(self):
    ...
    support_add_comment_output = support_add_comment(self.channel_id, self.case,
                                                     comment, self.user_id,
                                                     self.user_name)
    self.assertEqual(support_add_comment_output, None)

  def step21_support_change_priority(self):
    """
        Run the support_change_priority procedure. If successful, a message will
        appear in Slack.
    """
    priority = "P3"
  ...
  def step23_support_close_case(self):
    """
        ...
        appear in Slack.
    """
    support_close_case_output = support_close_case(self.channel_id, self.case,
                                                   self.user_id)
  ...
  def step27_asset_auto_cc(self):
    """
        Run the auto_asset_cc procedure.
    """
    asset_auto_cc_output = asset_auto_cc(
        self.channel_id, self.channel_name, "projects", self.project_number,
        self.user_id, ["test1@example.com", "test2@example.com"])
    self.assertEqual(asset_auto_cc_output, None)

  def step28_edit_asset_auto_cc(self):
    """
        Run the edit_asset_auto_cc procedure.
    """
    edit_asset_auto_cc_output = edit_asset_auto_cc(
        self.channel_id, self.channel_name, "projects", self.project_number,
        self.user_id, ["test300@example.com", "test302@example.com"])
    self.assertEqual(edit_asset_auto_cc_output, None)

  def step29_stop_asset_auto_cc(self):
    """
        Run the stop_asset_auto_cc procedure.
    """
    stop_asset_auto_cc_output = stop_asset_auto_cc(self.channel_id,
                                                   self.channel_name,
                                                   "projects",
                                                   self.project_number,
                                                   self.user_id)
  ...
  def step30_list_asset_auto_cc_subscriptions(self):
    """
        Run the list_asset_auto_cc_subscriptions procedure.
    """
    list_asset_auto_cc_subscriptions_output = list_asset_auto_cc_subscriptions(
        self.channel_id, self.channel_name)
    self.assertEqual(list_asset_auto_cc_subscriptions_output, None)

  def _steps(self):
    """
        Yields the methods in this class in sorted order so we can perform our
        integration test
    """
