...
from google.cloud import exceptions, pubsub, storage
from google.cloud.storage import iam
from googleapiclient import discovery
from yaspin import yaspin

from gcs_bucket_mover import bucket_details, configuration, sts_job_status
...
def move_bucket(conf):
    ...
    with yaspin(text='Create temp target bucket') as spinner:
        ...
        spinner.write(CHECKMARK + ' Bucket {} created in target project {}'.
                      format(config.temp_bucket_name, config.target_project))
    ...
    with yaspin(
            text='Assign STS permissions to source/temp buckets') as spinner:  # Deferring to yapf style. pylint: disable=bad-continuation
    ...
    with yaspin(
            text='Remove STS permissions from new source bucket') as spinner:  # Deferring to yapf style. pylint: disable=bad-continuation
...
def _get_project_number(project_id, credentials):
    """Using the friendly project name, get the unique project number for a project.
    ...
    project = crm.projects().get(projectId=project_id).execute()  # pylint: disable=no-member
...
def _create_bucket(spinner, config, storage_client, bucket_name,
    ...
    try:
        ...
        spinner.write(
            '503 returned, sleeping for 5s before trying bucket creation one more time'
        )
        sleep(5)
        bucket.create()
...
def _update_acl_entities(config, source_entities):
    """Take the existing ACLs, replace the source project number with the target project
    number and then assign the ACLs to the new bucket.
    ...
    for entity in source_entities:
        ...
        entity.identifier = entity.identifier.replace(
            source_project_number, target_project_number)
...
def _assign_sts_iam_roles(sts_email, storage_client, project_name, bucket_name,
                          assign_viewer):
    """Assign the roles to the STS service account that will be required to interact with the
    bucket.
...
def _print_sts_counters(spinner, counters, is_job_done):
    ...
    if counters:
        ...
        if is_job_done:
            if (bytes_copied_to_sink == bytes_found_from_source ==
                    bytes_deleted_from_source) and (
