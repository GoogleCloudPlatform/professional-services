...
"""Test method that sets up a bucket, adding objects and assigning various settings.
It will also attempt to delete the bucket from both the source and target projects before
it starts so that it can be run repeatedly.
"""
...
from gcs_bucket_mover import configuration, bucket_mover_service
...
def set_up_test_bucket(conf):
    """Sets up the test bucket, making sure none of the buckets already exist, and then runs the
    main script.
    ...
    with yaspin(text='TESTING: Cleanup source bucket') as spinner:
        ...
        source_bucket = create_bucket(config.source_storage_client,
                                      conf.bucket_name)
        spinner.write(bucket_mover_service.CHECKMARK +
                      ' TESTING: Bucket {} created in source project {}'.format(
                          conf.bucket_name, conf.source_project))
...
def create_bucket(storage_client, bucket_name):
    """Creates the test bucket, along with setting up lots of different bucket settings to make
    sure they can be moved.
...
def _check_bucket_exists_and_delete(spinner, storage_client, bucket_name,
                                    project_name):
    """Checks if the bucket exists and prompts the user to make sure they want to delete it and
    everything in it.
    ...
    if bucket.exists():
        ...
        answer = raw_input(
            ...
            ' do delete it: '.format(bucket_name, project_name))
        spinner.show()
        if answer != 'YES':
            spinner.fail('X')
            raise SystemExit()
        ...
        spinner.write(bucket_mover_service.CHECKMARK +
                      ' TESTING: Bucket {} deleted from'
                      ' project {}'.format(bucket_name, project_name))
