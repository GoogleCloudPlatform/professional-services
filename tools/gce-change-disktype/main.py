...
def main():
  ...
  create_newdisk_process(args.project, args.zone, args.instance, args.disktype, args.destructive)
...
def create_newdisk_process(project, zone, instance, disktype, destructive):
  ...
  disktype = 'https://www.googleapis.com/compute/v1/projects/mgtm-sandbox/zones/us-central1-a/diskTypes/' + disktype
  for source_disk in disks:
    ...
    snapshot_name = '{}-update-disk-{}'.format(existing_disk_name[0:39],int(datetime.datetime.now().timestamp()))
    new_disk_name = '{}-disk-{}'.format(existing_disk_name[0:46],int(datetime.datetime.now().timestamp()))
