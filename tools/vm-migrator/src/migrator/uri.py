...
class Instance(ProjectZone):
    ...
    def from_uri(cls, uri):
        ...
        match = re.search(
            r"(?:\/|^)projects\/(.*?)\/zones\/(.*?)\/instances\/(.*?)$", uri
        )
...
class Subnet(ProjectRegion):
    ...
    def from_uri(cls, uri):
        ...
        match = re.search(
            r"(?:\/|^)projects\/(.*?)\/regions\/(.*?)\/subnetworks\/(.*?)$", uri
        )
    ...
    @property
    def uri(self):
        return "projects/{}/regions/{}/subnetworks/{}".format(
            self.project, self.region, self.name
        )
...
class Disk(ProjectZone):
    ...
    @property
    def uri(self):
        return "projects/{}/zones/{}/disks/{}".format(
            self.project, self.zone, self.name
        )
...
class MachineType(ProjectZone):
    ...
    def from_uri(cls, uri):
        ...
        match = re.search(
            r"(?:\/|^)projects\/(.*?)\/zones\/(.*?)\/machineTypes\/(.*?)$", uri
        )
...
class MachineImage(Project):
    ...
    def from_uri(cls, uri):
        ...
        match = re.search(
            r"(?:\/|^)projects\/(.*?)\/global\/machineImages\/(.*?)$", uri
        )
