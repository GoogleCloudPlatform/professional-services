        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for future in concurrent.futures.as_completed(machine_image_future):
                try:
                    ...
                    tracker = tracker + 1
                    _LOGGER.info(
                        'Created machine image "%s" (%i/%i) in project "%s"',
                        machine_image_name,
                        tracker,
                        count,
                        project,
                    ...
                    _LOGGER.error(
                        'Could not create machine image "%s" in project "%s". Error: %s',
                        machine_image_name,
                        project,
                        exc,
                    )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for future in concurrent.futures.as_completed(instance_future):
                try:
                    ...
                    tracker = tracker + 1
                    _LOGGER.info(
                        'Deleted instance "%r" (%i/%i)', instance_name, tracker, count
                    )
                except Exception as exc:
                ...
                for i in range(9):
                    if row["disk_name_" + str(i + 1)] != "":
                        if instance_uri.name in deleted_instances:
                            ...
                            _LOGGER.info(
                                'Instance "%s" not deleted, preserving disk "%s"',
                                instance_uri,
                                row["disk_name_" + str(i + 1)],
                            )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for future in concurrent.futures.as_completed(instance_future):
                try:
                    ...
                    tracker = tracker + 1
                    _LOGGER.info(
                        'Shutdown machine "%r" (%i/%i)', machine_name, tracker, count
                    )
                except Exception as exc:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for future in concurrent.futures.as_completed(instance_future):
                try:
                    ...
                    tracker = tracker + 1
                    _LOGGER.info(
                        'Started machine "%r" (%i/%i)', machine_name, tracker, count
                    )
                except Exception as exc:
...
def bulk_move_instances_to_subnet(
    ...
    if direction != "backup" and direction != "rollback":
        _LOGGER.error(
            'bulk_move_instances_to_subnet: specify a direction ("backup" or "rollback"). Got: "%s"',
            direction,
        )
        return False
    result = True
    ...
    with open(file_name, "r") as read_obj:
        ...
        for row in csv_dict_reader:
            if not row["fingerprint"]:
                _LOGGER.error(
                    'Missing fingerprint for instance "%s", aborting', row["name"]
                )
                return False
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for row in csv_dict_reader:
                ...
                instance_future.append(
                    executor.submit(
                        instance.move_to_subnet_and_rename,
                        instance_uri,
                        row,
                        to_subnet_uri,
                        direction,
            ...
            for future in concurrent.futures.as_completed(instance_future):
                try:
                    ...
                    tracker = tracker + 1
                    _LOGGER.info(
                        'Moved machine "%r" (%i/%i) to subnet "%s"',
                        machine_name,
                        tracker,
                        count,
                        to_subnet_uri,
                    ...
                    _LOGGER.error(
                        'Could not move machine to subnet "%s". Error: %s',
                        to_subnet_uri,
                        exc,
                    )
                    result = False
...
def add_machineimage_iampolicies(
    file_name, source_project, target_service_account
) -> bool:
    result = True
    with open(file_name, "r") as read_obj:
        csv_dict_reader = DictReader(read_obj)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for future in concurrent.futures.as_completed(machineimage_future):
                try:
                    ...
                    tracker = tracker + 1
                    _LOGGER.info(
                        'Set IAM policy for service account "%s to machine image "%s" (%i/%i)',
                        target_service_account,
                        machine_name,
                        tracker,
                        count,
                    ...
                    _LOGGER.error(
                        'Could not set IAM policy for service account "%s". Error: %s',
                        target_service_account,
                        exc,
                    )
                    result = False
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for row in csv_dict_reader:
                ...
                for i in range(4):
                    ...
                    if row["range_name_" + str(i + 1)] != "":
                        alias_range["subnetworkRangeName"] = row[
                            "range_name_" + str(i + 1)
                        ]
                ...
                for i in range(9):
                    if row["device_name_" + str(i + 1)] != "":
                        disk_names[row["device_name_" + str(i + 1)]] = row[
                            "disk_name_" + str(i + 1)
                        ]
                    if row["disk_labels_" + str(i + 1)] != "":
                        ...
                        ] = row["disk_labels_" + str(i + 1)]

                node_group = None
                if row["node_group"] and row["node_group"] != "":
                    node_group = row["node_group"]

                source_machine_type_uri = uri.MachineType.from_uri(row["machine_type"])
                ...
                if target_subnet_uri.region != target_instance_uri.region:
                    _LOGGER.error(
                        'Instance mapping from "%s" to "%s" is outside the target region "%s"',
                        source_instance_uri,
                        target_instance_uri,
                        target_subnet_uri,
                    )
            ...
            for future in concurrent.futures.as_completed(instance_future):
                try:
                    ...
                    tracker = tracker + 1
                    _LOGGER.info(
                        'Created instance "%r" (%i/%i)', instance_name, tracker, count
                    )
                except Exception as exc:
            ...
            for disk_uri, labels in disk_labels.items():
                disk_future.append(
                    executor.submit(
                        disk.setLabels, uri.Disk.from_uri(disk_uri), json.loads(labels)
                    )
                )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for row in csv_dict_reader:
                ...
                disable_deletionprotection_future.append(
                    executor.submit(
                        instance.disable_deletionprotection, source_instance_uri
                    )
                )
                count = count + 1
            ...
            for future in concurrent.futures.as_completed(
                ...
                try:
                    ...
                    tracker = tracker + 1
                    _LOGGER.info(
                        'Disabled deletion protection for instance "%s" (%i/%i)',
                        instance_name,
                        tracker,
                        count,
                    )
                    ...
                    _LOGGER.error(
                        "Could not disable deletion protection. Error: %s", exc
                    )
                    result = False
    return result
...
def query_yes_no(question, default="yes"):
    ...
    if default is None:
        ...
        raise ValueError(
            f'Invalid default answer "{default}" for question "{question}"'
        )
...
def filter_records(source_file, filter_file, destination_file):
    ...
    if os.path.exists(filter_file):
        ...
        with open(source_file, "r") as csvfile:
            ...
            for row in csv_dict_reader:
                if row["name"] in machine_names_to_filter:
                    filtered.append(row)
        _LOGGER.info(
            'There are %d entries to be writen into "%s"',
            len(filtered),
            destination_file,
        )
        if overwrite_file:
        ...
        _LOGGER.warning(
            'There is no filter file "%s". All entries in "%s" will be copied over into "%s".',
            filter_file,
            source_file,
            destination_file,
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for future in concurrent.futures.as_completed(releaseip_future):
                try:
                    ...
                    tracker += 1
                    _LOGGER.info(
                        "[%s] Releasing individual IPs (%i/%i)",
...
def main(step, migration_json) -> bool:
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not read content of '{migration_json}' as a JSON file. Error: {err}"
        ) from err

    _LOGGER.info(
        "[Step: '%s'] Using the following migration config: '%s'", step, config
    )
    ...
    for migration_map in config.get("mapping", []):
        ...
        if skip == step:
            _LOGGER.info(
                "[Step: '%s'] Skipping migration unit: '%s' (skip='%s')",
                step,
                migration_map,
                skip,
            )
        ...
        target_service_account_scopes = migration_map.get(
            "target_service_account_scopes"
        )
        target_scopes = (
            ",".join(target_service_account_scopes)
            if target_service_account_scopes
            else ""
        )
...
# main function
def single_subnet_main(
    step,
    machine_image_region,
    source_project,
    source_subnet_uri: uri.Subnet,
    source_zone,
    source_zone_2,
    source_zone_3,
    target_project,
    ...
    if step == "prepare_inventory":
        ...
        if subnet.list_instances_for_rollback(
            source_project, source_zone, backup_subnet_uri, input_csv, rollback_csv
        ):
        ...
        if disable_delete_protection_response:
            ...
            if bulk_instance_disable_deletionprotection(input_csv):
                _LOGGER.info(
                    "Successfully disabled deletion protection for all instances"
                )
            else:
                _LOGGER.warning(
                    "Disabling deletion protection for all instances failed"
                )
                return False
        else:
        ...
        if response:
            _LOGGER.info(
                "Deleting all the instances and disks present in the inventory"
            )
            if bulk_delete_instances_and_disks(input_csv, source_project):
                _LOGGER.info(
                    "Successfully deleted all instances and disks present in the inventory"
                )
            else:
                _LOGGER.warning(
                    "Deleting all instances and disks in the inventory failed"
                )
                return False
        else:
        ...
        _LOGGER.info(
            'Setting IAM policies of created machine images with input_csv="%s", source_project="%s", '
            'target_service_account="%s"',
            input_csv,
            source_project,
            target_service_account,
        )
        if add_machineimage_iampolicies(
            input_csv, source_project, target_service_account
        ):
        ...
        _LOGGER.info(
            'Creating instances retaining the original ips in file "%s" with source_project="%s", '
        ...
        _LOGGER.info(
            'Releasing all IPs of project "%s" present in subnet "%s"',
            source_project,
            source_subnet_uri,
        )
        if subnet.release_ip(source_project, source_subnet_uri):
            _LOGGER.info(
                'All IPs of project "%s" present in subnet "%s" released successfully',
                source_project,
                source_subnet_uri,
            )
        else:
            _LOGGER.error(
                'Releasing the IPs of project "%s" present in subnet "%s" failed',
                source_project,
                source_subnet_uri,
            )
            return False
        ...
        if release_individual_ips(source_subnet_uri, input_csv):
            _LOGGER.info(
                'IPs present in the file "%s" released successfully', input_csv
            )
        else:
