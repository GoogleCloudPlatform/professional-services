...
def get_alias_ip_name(instance_uri: uri.Instance, subnet_uri: uri.Subnet, ip):
    ...
    ips = (
        compute.addresses()
        .list(
            project=instance_uri.project, region=instance_uri.region, filter=ip_filter
        )
        .execute()
    )
    if ips.get("items") and len(ips.get("items")) == 1:
        ...
        _LOGGER.info(
            'Alias ip "%s" was not reserved for instance "%s" in subnet "%s"',
            ip,
            instance_uri,
            subnet_uri,
        )
...
def export_instances_by_subnets(
    project: str, subnet_to_zones: Dict[uri.Subnet, List[str]], file_name: str
) -> bool:
    _LOGGER.info(
        'Exporting instances from project "%s", subnets "%s", and zones "%s" into file "%s"',
        project,
    ...
    for subnet, zones in subnet_to_zones.items():
        for zone in zones:
            ...
            if not done:
                _LOGGER.warning(
                    'Could not export instances from project "%s", subnet "%s", and zone "%s" '
                    'into file "%s". Skipping.',
                    project,
                    subnet,
                    zone,
                    file_name,
...
def export_instances_by_subnet_in_zones(
    project: str, zones: List[str], subnet_uri: uri.Subnet, file_name: str
) -> bool:
    _LOGGER.info(
        'Exporting instances from project "%s", subnet "%s", and zones "%s" into file "%s"',
        project,
        subnet_uri,
        zones,
        file_name,
    ...
    for zone in zones:
        ...
        if not done:
            _LOGGER.warning(
                'Could not export instances from project "%s", subnet "%s", and zone "%s" into file "%s". '
                "Skipping.",
                project,
                subnet_uri,
                zone,
...
def _export_instances_by_subnet_zone(
    project: str, zone: str, subnet_uri: uri.Subnet, file_name: str
) -> bool:
    compute = get_compute()
    _LOGGER.info(
        'Fetching the inventory for the source subnet "%s" and zone "%s" in project "%s". Results in "%s"',
        subnet_uri,
        zone,
        project,
        file_name,
    ...
    result = (
        compute.instances().list(project=project, zone=zone, maxResults=10000).execute()
    )
    if not result.get("items"):
        result = {"items": []}
    mydict = {}
    _LOGGER.info(
        'Identified %i potential instance(s) in the given zones "%s" in subnet "%s" in project "%s"',
        len(result["items"]),
        zone,
        subnet_uri,
        project,
    ...
    for instances in result["items"]:
        if instances["networkInterfaces"][0]["subnetwork"].endswith(subnet_uri.uri):
            csv = {
                "name": instances["name"],
                "id": instances["id"],
            ...
            if alias_ips:
                ...
                for i in range(len(alias_ips)):
                    ...
                    ip_name = get_alias_ip_name(
                        instance_uri, subnet_uri, alias_ips[i]["ipCidrRange"]
                    )
                    if ip_name:
                        csv["alias_ip_name_" + str(i + 1)] = ip_name

                    if alias_ips[i].get("subnetworkRangeName"):
                        csv["range_name_" + str(i + 1)] = alias_ips[i][
                            "subnetworkRangeName"
                        ]
            ...
            if fingerprint:
                _LOGGER.info(
                    'Found instance nic0 fingerprint for "%s"', instances["name"]
                )
            ...
            _LOGGER.debug(
                'Ignoring VM "%s" in subnet "%s" (looking for subnet "%s")',
                instances["name"],
    ...
    _LOGGER.info(
        'Fetching disks for the source subnet "%s" and zone "%s" in project "%s"',
        subnet_uri,
        zone,
        project,
    )
    result = (
        compute.disks().list(project=project, zone=zone, maxResults=10000).execute()
    )
    if not result.get("items"):
        result = {"items": []}
    ...
    for disks in result["items"]:
        ...
        for i in range(9):
            if mydict[instance_link]["disk_name_" + str(i + 1)] == disks["name"]:
                mydict[instance_link]["disk_labels_" + str(i + 1)] = json.dumps(
                    disks["labels"]
                )
                break
...
def list_instances_for_rollback(
    project, zone, backup_subnet_uri: uri.Subnet, previous_instances_file, to_file
):
    ...
    try:
        ...
        _LOGGER.error(
            'Cannot read file "%s" and get previous IPs of instances. Error: %s',
            previous_instances_file,
            exc,
        )
        return False
    if not internal_ips:
        _LOGGER.error(
            'Can not find previous IPs of instances in file "%s"',
            previous_instances_file,
        )
        return False
    compute = get_compute()
    ...
    _LOGGER.info(
        'Fetching inventory for source subnet "%s" and zone "%s" in project "%s"',
        backup_subnet_uri,
        zone,
        project,
    )
    ...
    _LOGGER.info(
        'Identified %i potential instance(s) in the given zone "%s" in project "%s"',
        len(result["items"]),
        zone,
        project,
    )
    ...
    for instances in result["items"]:
        if instances["networkInterfaces"][0]["subnetwork"].endswith(
            ...
            if instances["id"] not in internal_ips:  # previous IP not found
                _LOGGER.error(
                    'No previous IP found for instance "%s"', instances["name"]
                )
                return False
...
def release(project_region_uri: uri.ProjectRegion, address) -> bool:
    ...
    try:
        _LOGGER.info(
            'Releasing IP address "%s" in project "%s"', address, project_region_uri
        )
        result = (
            compute.addresses()
            .delete(
        ...
        _LOGGER.error(
            'Error while releasing IP address "%s" in project "%s". Error: %s',
            address,
            project_region_uri,
            err,
        )
...
def release_individual_ips(
    subnet_uri: uri.Subnet, instance_uri: uri.Instance, ips
) -> bool:
    result = True
    compute = get_compute()
    for ip in ips:
        ips_result = (
            ...
            .list(
                ...
                filter='(address="{}") AND (subnetwork="{}")'.format(
                    ip, subnet_uri.abs_beta_uri
                ),
            )
            .execute()
        )
        if "items" in ips_result and 1 == len(ips_result["items"]):
            ...
            _LOGGER.info(
                'Deletion of internal ip "%s" in subnet "%s" for instance "%s" '
...
def release_ip(project: str, subnet_uri: uri.Subnet) -> bool:
    ...
    if ips.get("items"):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for addresses in ips["items"]:
                ...
                releaseip_future.append(
                    executor.submit(
                        release, uri.ProjectRegion(project, subnet_uri.region), ip_name
                    )
                )
                count = count + 1
            tracker = 0
            for future in concurrent.futures.as_completed(releaseip_future):
                try:
                    ...
                    tracker += 1
                    _LOGGER.info(
                        '[] Releasing %i out of %i IPs in subnet "%s" in project "%s"',
                        "DONE" if sub_result else "FAILED",
                        tracker,
                        count,
                        sub_result,
                        project,
                    ...
                    _LOGGER.error(
                        'Error releasing IPs "%s" in project "%s" subnet "%s". Error: %s',
                        ips.get("items"),
                        project,
                        subnet_uri,
                        exc,
        ...
        _LOGGER.warning(
            'No reserved internal IP addresses found in the subnet "%s" from project "%s"',
            subnet_uri,
            project,
        )
    return result
...
def wait_for_operation(
    compute, project_region_uri: uri.ProjectRegion, operation
) -> object:
    _LOGGER.info(
        'Waiting for operation "%s" to finish in "%s"', operation, project_region_uri
    )
    while True:
        result = (
        ...
        if result["status"] == "DONE":
            _LOGGER.info(
                'Finished operation "%s" in "%s"', operation, project_region_uri
            )
            if "error" in result:
                _LOGGER.info(
                    'Error in operation "%s" in "%s". Results: %s',
                    operation,
                    project_region_uri,
                    result,
                )
...
def duplicate(source_subnet_uri: uri.Subnet, target_subnet_uri: uri.Subnet) -> bool:
    compute = get_compute()
    subnet_request = compute.subnetworks().get(
        project=source_subnet_uri.project,
    ...
    try:
        ...
        _LOGGER.error(
            'Could not delete subnetwork "%s". Error: %s', source_subnet_uri, err
        )
        return False
    ...
    if (
        "enableFlowLogs" not in config or not config["enableFlowLogs"]
    ...
    insert_operation = (
        compute.subnetworks()
        .insert(
            project=target_subnet_uri.project,
    ...
    try:
        ...
        _LOGGER.error(
            'Could not recreate subnetwork "%s". Error: %s', target_subnet_uri, err
        )
        return False
    return True
...
def get_network(subnet_uri: uri.Subnet):
    result = (
        ...
        .subnetworks()
        .get(
            project=subnet_uri.project,
            region=subnet_uri.region,
            subnetwork=subnet_uri.name,
        )
        .execute()
    )
