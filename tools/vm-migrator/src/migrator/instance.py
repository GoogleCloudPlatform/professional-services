    ...
    try:
        if instance.get("scheduling"):
            if instance.get("scheduling").get("nodeAffinities"):
                if (
                    isinstance(instance["scheduling"]["nodeAffinities"], list)
                    and len(instance["scheduling"]["nodeAffinities"]) > 0
                ):
                    if (
                        instance["scheduling"]["nodeAffinities"][0].get("key")
        ...
        _LOGGER.warning(
            'Could not check if instance "%s" is hosted in sole tenant. Error: %s',
            instance,
            err,
        )
        return False
...
def get_updated_node_group(node_group) -> Optional[object]:
    ...
    try:
        if node_group_mapping.FIND.get(node_group):
            ...
            _LOGGER.info(
                "Found a matching node group %s for %s",
                node_group,
...
def disable_deletionprotection(instance_uri: uri.Instance) -> str:
    ...
    try:
        ...
        if ex.status_code == 404:
            ...
            _LOGGER.error(
                'Could not disable delete protection in instance "%s". Error: %s',
                instance_uri,
                ex,
            )
            raise ex
...
def wait_for_zonal_operation(compute, project_zone_uri: uri.ProjectZone, operation):
    _LOGGER.info(
        'Waiting for zonal operation "%s" in project "%s" to finish',
        operation,
        project_zone_uri,
    )
    while True:
        result = (
            ...
            .get(
                project=project_zone_uri.project,
        ...
        if result["status"] == "DONE":
            _LOGGER.info(
                'Finished zonal operation "%s" in project "%s"',
                operation,
                project_zone_uri,
            )
            if "error" in result:
                _LOGGER.error(
                    'Error in zonal operation "%s" in project "%s". Result: "%s"',
                    operation,
                    project_zone_uri,
                    result,
                )
...
def wait_for_regional_operation(
    compute, project_region_uri: uri.ProjectRegion, operation
):
    _LOGGER.info(
        'Waiting for regional operation "%s" in project "%s" to finish',
        operation,
        project_region_uri,
    )
    while True:
        ...
        if result["status"] == "DONE":
            _LOGGER.info(
                'Finished regional operation "%s" in project "%s"',
                operation,
                project_region_uri,
            )
            if "error" in result:
                _LOGGER.error(
                    'Error in regional operation "%s" in project "%s". Result: "%s"',
                    operation,
                    project_region_uri,
                    result,
                )
...
def delete(instance_uri: uri.Instance) -> str:
    ...
    try:
        ...
        if image:
            _LOGGER.info(
                'Found machine image "%s", can safely delete the instance "%s"',
                image,
                instance_uri,
            )
            ...
            raise NotFoundException(
                f'Cannot delete instance "{instance_uri}" as machine image not found'
            )
    except googleapiclient.discovery.HttpError as ex:
        if ex.status_code == 404:
            _LOGGER.warning(
...
def get_ip(compute, instance_uri: uri.Instance, address) -> str:
    result = (
        compute.addresses()
        .get(project=instance_uri.project, region=instance_uri.region, address=address)
        .execute()
    )
    return result["address"]


def reserve_internal_ip(
    compute, instance_uri: uri.Instance, name, subnet_uri: uri.Subnet, ip
) -> str:
    config = {
        "address": ip,
    ...
    _LOGGER.debug(
        'Reserving internal ip "%s" with name "%s" for instance "%s" in subnet "%s"',
        ip,
        name,
        instance_uri,
        subnet_uri,
    ...
    try:
        insert_operation = (
            compute.addresses()
            .insert(
                project=instance_uri.project, region=instance_uri.region, body=config
            )
            .execute()
        )
...
def prepare_create_instance(
    ...
    config = {
        ...
        "networkInterfaces": [
            {"network": network, "subnetwork": subnet_uri.uri, "networkIP": ip}
        ],
        "disks": [
            {"deviceName": device_name, "initializeParams": {"diskName": disk_name}}
    ...
    if ip:
        _LOGGER.info(
            'Trying to create instance "%s" while preserving its ips', instance_uri
        )
        ...
        config["networkInterfaces"][0]["networkIP"] = get_ip(
            compute, instance_uri, instance_uri.name
        )

    if node_group and get_updated_node_group(node_group):
        _LOGGER.info(
            'Found a sole tenant matching running on node group "%s"', node_group
        )
    ...
    if len(alias_ip_ranges) > 0:
        ...
        for alias_ip in alias_ip_ranges:
            ...
            if not alias_ip.get("subnetworkRangeName") and alias_ip[
                "ipCidrRange"
            ].endswith("/32"):
                ...
                _LOGGER.info(
                    'Reserving alias "%s" for ip "%s" for instance "%s" in subnet "%s"',
                    alias_ip_name,
                    actual_ip,
                    instance_uri,
                    subnet_uri,
                ...
                reserve_internal_ip(
                    compute, instance_uri, alias_ip_name, subnet_uri, actual_ip
                )
                # Since we are not retaining the ip we will use the
                # random reserved ip
                if not actual_ip:
                    alias_ip["ipCidrRange"] = (
                        get_ip(compute, instance_uri, alias_ip_name) + "/32"
                    )
                i = i + 1
        config["networkInterfaces"][0]["aliasIpRanges"] = alias_ip_ranges

    return {"project": instance_uri.project, "zone": instance_uri.zone, "body": config}
...
def wait_for_instance(compute, instance_uri: uri.Instance):
    ...
    while True:
        ...
        if result["status"] == "RUNNING":
            ...
            if "error" in result:
                _LOGGER.error(
                    'Instance "%s" did not start. Result: %s', instance_uri, result
                )
                raise GCPOperationException(result["error"])
            return result
...
def move_to_subnet_and_rename(
    instance_uri: uri.Instance, row, to_subnet_uri, direction: str
) -> str:
    _LOGGER.info("Moving to subnet and renaming with: %s", locals())
    if direction not in ["backup", "rollback"]:
        _LOGGER.error(
            'move_to_subnet_and_rename: specify a direction("backup" or "rollback"). Got "%s"',
            direction,
        )
        return False
    ...
    try:
        ...
        compute = get_compute()
        _LOGGER.debug(
            'Updating network interface "%s" in row "%s" for instance "%s"',
            to_subnet_uri,
            row,
            instance_uri,
        )
        ...
        kwargs["body"] = {"currentName": kwargs["instance"], "name": new_name}
...
def create(
    ...
    compute = get_compute()
    _LOGGER.debug(
        'Creating instance "%s" in network "%s", subnet "%s", '
    ...
    kwargs = prepare_create_instance(
        compute,
        instance_uri,
        network,
        subnet,
    ...
    while True:
        try:
            if 0 < retry_count:
                ...
                _LOGGER.info(
                    'Retry #%d: Creating instance "%s", waiting "%d"',
                    retry_count,
                    instance_uri,
                    wait_time,
                )
            ...
            _LOGGER.info(
                'Instance "%s" created from source instance "%s" and successfully started',
                created_instance_uri,
                instance_uri,
            )
            ...
            if (
                isinstance(ex, googleapiclient.discovery.HttpError)
                and ex.status_code == 409
            ):
                _LOGGER.warning(
                    'Instance "%s" already exists. Ignoring. Error: %s',
                    instance_uri,
                    ex,
                )
                break
