...
def filter_records(source_file, filter_file, destination_file):
    ...
    overrtie_file = query_yes_no(
        "About to overrite %s with %i records, please confirm to continue" %
...
def main(project,
    ...
    if (step == 'filter_inventory'):
        ...
        overrite_file = filter_records("source.csv", filter_file_name,
                                       file_name)
        if (overrite_file):
            logging.info("%s now has filtered records" % (file_name))
        else:
            logging.info("File %s was not overriten" % (file_name))
