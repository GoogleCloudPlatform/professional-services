...
def label(self_link, access_token, data):
    ...
    try:
        req = requests.post("%s/setLabels" % self_link,
                            headers=headers,
...
def main(argv):
    ...
    instance = conn.instances().get(project=project,
                                    zone=zone,
    ...
    label(
        instance['selfLink'],
        creds.get_access_token().access_token,
    ...
    for i in instance['disks']:
        ...
        if 'source' not in i:
            continue
        disk = conn.disks().get(project=project,
                                zone=zone,
