CREATE OR REPLACE PROCEDURE `<project_id>.DR_ADMIN.crr_init`(IN var_project_id STRING, IN var_execution_region_id STRING, OUT p_error_load_batch_id STRING, OUT p_out_status STRING)
BEGIN
...
CALL `<project_id>.DR_ADMIN.crr_associate`(var_project_id, var_execution_region_id, p_error_load_batch_id, p_out_status);
...
CREATE OR REPLACE PROCEDURE `DR_ADMIN.crr_associate`(IN var_project_id STRING, IN var_execution_region_id STRING, OUT p_error_load_batch_id STRING, OUT p_out_status STRING)
BEGIN
...
--Purpose: This Procedure initiates the creation of the replica for each of the CRR datasets. This Procedure will also initiates the deletion of the replica which is no longer part of the CRR Solution
/*
DECLARE var_project_id STRING DEFAULT '<project_id>';
DECLARE var_execution_region_id STRING DEFAULT 'us-central1';
DECLARE p_error_load_batch_id STRING DEFAULT NULL;
DECLARE p_out_status STRING DEFAULT NULL;
*/
  ...
  DECLARE var_crr_error_bucket_name STRING;
  DECLARE var_config_project_id STRING;
  DECLARE var_reservation_name STRING;
  DECLARE var_error_count INT64;
...
SET (var_config_project_id,var_reservation_name) = (SELECT AS STRUCT project_id,concat(failover_reserrvation_project,".",failover_reservation_name) as reservation_name from DR_ADMIN.replica_inscope_projects where lower(project_id) = lower(var_project_id));

-- Validate whether the project is in scope
IF var_config_project_id is null or var_reservation_name is null THEN
 RAISE USING MESSAGE = 'Replica Inscope Projects Table is not fully populated';
END IF;
...
--Retrieve the replication metadata for the current project and region
EXECUTE IMMEDIATE """ CREATE OR REPLACE TEMP TABLE schemata_replicas as 
SELECT  schema_name,failover_reservation_project_id,creation_complete,location FROM `"""||var_project_id||".region-"||var_primary_region||"""`.INFORMATION_SCHEMA.SCHEMATA_REPLICAS  """ ;
...
--Retrieve the current replication status of the datasets 
EXECUTE IMMEDIATE """
CREATE OR REPLACE TEMP TABLE reservation_disassociation as
select schema_name as dataset_name from schemata_replicas
where creation_complete=true
and location in ('"""||var_primary_region||"""','"""||var_secondary_region||"""')
and failover_reservation_project_id is null
group by schema_name
having count(*) >1
""" ;

EXECUTE IMMEDIATE """
CREATE OR REPLACE TEMP TABLE reservation_association as
select schema_name as dataset_name from schemata_replicas
where creation_complete=true
and location in ('"""||var_primary_region||"""','"""||var_secondary_region||"""')
and failover_reservation_project_id is not null
group by schema_name
having count(*) >1
""" ;

--Retrieve only the datasets which needs to be associated
CREATE OR REPLACE TEMP TABLE dataset_associate_list as
select rds.dataset_name from reservation_disassociation rds
inner join dataset_boarding dbr
on rds.dataset_name =dbr.dataset_name
and dbr.active_flag = true ;

CREATE OR REPLACE TEMP TABLE dataset_disassociate_list as
select ras.dataset_name from reservation_association ras
inner join dataset_boarding dbr
on ras.dataset_name =dbr.dataset_name
and dbr.active_flag = false ;

--Associate the dataset from the association list
FOR active in 
  ( 
 select distinct dataset_name from dataset_associate_list
)
  DO
  --Send each dataset to the association process
CALL DR_ADMIN.dataset_associator(var_project_id, active.dataset_name,true, var_reservation_name,var_primary_region, p_error_load_batch_id, p_out_status);
END FOR;

--Dissociate the dataset from the disassociation list
FOR inactive in 
  ( 
 select distinct dataset_name from dataset_disassociate_list
)
  DO
  --Send each dataset to the association process
  CALL DR_ADMIN.dataset_associator(var_project_id, inactive.dataset_name,false, var_reservation_name,var_primary_region, p_error_load_batch_id, p_out_status);
END FOR;
...
IF var_error_count > 0 THEN
  EXPORT DATA
  OPTIONS (
   uri = CONCAT('gs://',var_crr_error_bucket_name,'/',var_project_id,'/association/',p_error_load_batch_id,'-error-*.csv'),
    format = 'CSV',
    overwrite = true,
    header = true,
    field_delimiter = ';')
...
EXPORT DATA
  OPTIONS (
   uri = CONCAT('gs://',var_crr_error_bucket_name,'/',var_project_id,'/association/',p_error_load_batch_id,'-error-*.csv'),
    ...
    field_delimiter = ';')
AS (
SELECT p_error_load_batch_id as batch_id,var_project_id as project_id,var_execution_region_id as execution_region_id, NULL as dataset_name,last_job_id as bq_job_id,'ASSOCIATION' as crr_job_type,'DR_ADMIN.crr_associate' as error_location,REPLACE(@@error.statement_text,'\n','') as error_statement,REPLACE(@@error.message,'\n','') as error_message,CURRENT_TIMESTAMP as metadata_created_time,NULL as metadata_comment
);
SET p_out_status  = "FAILED:Execution Error";
...
CREATE OR REPLACE PROCEDURE `DR_ADMIN.dataset_associator`(IN var_project_id STRING, IN var_dataset_name STRING, IN var_active_flag BOOL, IN var_reservation_name STRING, IN var_primary_region STRING, IN p_error_load_batch_id STRING, OUT p_out_status STRING)
BEGIN
...
--Validation of the input arguments
IF var_project_id is null or var_dataset_name is null or var_active_flag is null or var_reservation_name is null or p_error_load_batch_id is null THEN
 RAISE USING MESSAGE = 'One or more of the input argument is not fully populated';
END IF;

--Verify that the dataset is in active state for crr process
IF var_active_flag = true THEN
     SET sql_statmnt="ALTER SCHEMA `"||var_project_id||"`."||var_dataset_name||" SET OPTIONS(failover_reservation='"||var_reservation_name||"')";
  ELSE
     SET sql_statmnt="ALTER SCHEMA `"||var_project_id||"`."||var_dataset_name||" SET OPTIONS(failover_reservation=null)";
  END IF;
...
BEGIN
    ...
    END IF;
INSERT INTO  dr_error_details (batch_id,project_id,execution_region_id,dataset_name,bq_job_id,crr_job_type,error_location,error_statement,error_message,metadata_created_time,metadata_comment )
SELECT p_error_load_batch_id as batch_id,var_project_id as project_id,var_primary_region as execution_region_id, var_dataset_name as dataset_name,last_job_id as bq_job_id,'ASSOCIATION' as crr_job_type,'DR_ADMIN.dataset_associator' as error_location,REPLACE(@@error.statement_text,'\n','') as error_statement,REPLACE(@@error.message,'\n','') as error_message,CURRENT_TIMESTAMP as metadata_created_time,NULL as metadata_comment;
  SET p_out_status  = "FAILED:Execution Error";
END;
END;
