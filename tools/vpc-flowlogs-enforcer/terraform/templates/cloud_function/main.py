...
def enable_net_logs(event, context):
    ...
    logging.info('subnet ID: /projects/%s/regions/%s/subnetworks/%s.', project, region, subnetwork)
    ...
    if logs_enabled:
        logging.info('log config already enabled in subnetwork /projects/%s/regions/%s/subnetworks/%s.', project, region, subnetwork)
    else:
        logging.info('enabling flow logs in subnetwork /projects/%s/regions/%s/subnetworks/%s.', project, region, subnetwork)
        ...
        if 'status' in patch_response and patch_response['status'] == 'DONE':
            logging.info('flow logs successfully enabled in subnetwork /projects/%s/regions/%s/subnetworks/%s.', project, region, subnetwork)
