...
def get_gke_cluster(env_name, project_id, location):
  ...
  res = composer.projects().locations().environments().get(name=path).execute()
  cluster_path = re.search(
  r'projects\/(?P<project>.*)\/zones\/(?P<zone>.*)\/clusters\/(?P<cluster>.*)',
...
def extract_worker_pod_by_namespace(k8s_client, namespace):
  ...
  pod_name_pattern = r'^airflow-worker-([A-Za-z1-9]+)-([A-Za-z1-9]+)$'
...
def extract_composer_namespace(k8s_client):
  ...
  ns_pattern = r'^composer-(\d+)-(\d+)-(\d+)-airflow-(\d+)-(\d+)-(\d+)-([A-Za-z0-9]+)$'
  for ns in all_namespaces.items:
    if(re.match(ns_pattern, ns.metadata.name) and ns.status.phase == 'Active'):
...
def extract_airflow_secrets_by_namespace(k8s_client, namespace):
  secrets = k8s_client.read_namespaced_secret(
      namespace=namespace, name='airflow-secrets')
  if all(k in secrets.data.keys() for k in ('fernet_key', 'sql_password')):
    return {
      'fernet_key': base64.b64decode(
        secrets.data['fernet_key']).decode('utf-8'),
      'sql_password': base64.b64decode(
        secrets.data['sql_password']).decode('utf-8')
    }
...
def extract_sql_proxy_pod_id(k8s_client):
  ...
  sql_proxy_pod_pattern = r'airflow-sqlproxy-([A-Za-z1-9]+)-([A-Za-z1-9]+)'
  for pod in pods.items:
    if re.match(sql_proxy_pod_pattern, pod.metadata.name):
      return pod.status.pod_ip
  raise ValueError('Airflow SQL proxy pod not found')

def rotate_fernet_key(pod_name, namespace, source_fernet_key, destination_fernet_key):
  command = [
    'kubectl',
    'exec',
    '-t',
    ...
    'bash',
    '-c',
    f'export AIRFLOW__CORE__FERNET_KEY="{destination_fernet_key},{source_fernet_key}" && airflow rotate-fernet-key'
    ]
