...
class ComposerEnv(object):
  ...
  def from_current_context(cls):
    ...
    worker_pod_vars['AIRFLOW__CORE__FERNET__KEY'] = namespace_secrets['fernet_key']
  ...
  def get_dag_folder_path(self) -> str:
    """
    Returns the full GCS path associated with the dag folder of the current env
    """
    return f'gs://{self.worker_pod_vars["GCS_BUCKET"]}/dags/'

  def get_plugins_folder_path(self) -> str:
    """
    Returns the GCS path associated with the dag folder of the current env
    """
