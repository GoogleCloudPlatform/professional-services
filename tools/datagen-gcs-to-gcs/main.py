...
import os
import shutil
from google.cloud import bigquery # Removed storage import, individual modules handle it
...
def main():
    ...
    try:
        ...
        gemini_model = GenerativeModel("gemini-1.5-pro-002", generation_config=GenerationConfig(temperature=0))
    ...
    try:
        ...
        if config_vars.SOURCE_TYPE == "BigQuery":
            ...
            if not input_gcs_path: # Check if the dictionary is empty
                raise Exception("Failed to export BigQuery tables or input_gcs_path map is empty.")
        ...
        if not table_attributes or not any(attrs.get("staging_gcs_path") for attrs in table_attributes.values()):
            raise Exception("File preprocessing failed or no staging paths were generated.")

        # 3. Start Audit Log
        print("\\n--- Logging start entries to audit table ---")
        audit_utils.start_audit_log(bq_client_main, batch_id, input_gcs_path, table_attributes,header_gcs_path)
        ...
        local_snowfakery_output_batch = os.path.join(local_snowfakery_output_base, str(batch_id))
        ...
        audit_utils.end_audit_log(bq_client_main, batch_id, input_gcs_path, table_attributes,header_gcs_path)
        ...
        if 'local_snowfakery_output_batch' in locals() and os.path.exists(local_snowfakery_output_batch):
            try:
                shutil.rmtree(local_snowfakery_output_batch)
                print(f"Cleaned up local directory: {local_snowfakery_output_batch}")
            except Exception as cleanup_error:
                print(f"Error cleaning up local directory {local_snowfakery_output_batch}: {cleanup_error}")
