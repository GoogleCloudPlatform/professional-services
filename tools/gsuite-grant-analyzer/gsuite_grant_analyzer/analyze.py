...
from gsuite_grant_analyzer.bigquery_helpers import (
    bq_create_client, bq_create_dataset, bq_create_table, bq_insert_rows)
...
logging.basicConfig(
    filename='gsuite_grant_analyzer.log', level=logging.DEBUG,
...
def parse_args(args):
    ...
    parser.add_argument(
        '--domain', help="Domain", type=str, required=True)
    parser.add_argument(
        '--ou', help="Organizational Unit path, default is /",
        type=str, required=False, default='/')
    parser.add_argument(
        '--sa', help="Path to Service Account credentials",
        type=str, required=True)
    parser.add_argument(
        '--admin', help="Administrator email",
        type=str, required=True)
    parser.add_argument(
        '--project', help="Project where BigQuery dataset will be created",
        type=str, required=True)
    parser.add_argument(
        '--dry-run', help="Do not load data into BigQuery",
        required=False, dest='dry_run', action='store_true', default=False)
    parser.add_argument(
        '--processes', help="Number of concurrent processes (default: 4)",
        type=int, required=False, dest='num_processes', default=4)
...
def build_rows_process(queue, users, directory_client, process_number,
    ...
    for i, user in enumerate(users, 1):
        ...
        if i % num_processes == process_number:
            logger.debug(
                "Process #%s - User %d/%d: %s",
                process_number, i, total_users, user)
            rows.extend(
                get_denormalized_scopes_for_user(directory_client, user))
...
def analyze():
    ...
    for process_number in range(0, args.num_processes):
        ...
        process = Process(
            target=build_rows_process, args=(
                queue, users, directory, process_number, args.num_processes
                ))
    ...
    while processes:
        ...
        if message_type == 'DATA':
            ...
            if not args.dry_run and not terminate_processes.value:
                logger.info(
                    "Inserting data from process #%d", process_number)
                data = message['data']
                total_rows += len(data)
            ...
            logger.error(
                "Unknown message %s from process", message['message_type'])
            exit(1)
