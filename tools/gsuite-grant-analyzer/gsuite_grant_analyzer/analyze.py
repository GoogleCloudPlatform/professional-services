...
logging.basicConfig(
    filename="gsuite_grant_analyzer.log",
    level=logging.DEBUG,
...
def parse_args(args):
  ...
  parser.add_argument(
      ...
      required=True)
  parser.add_argument(
      "--admin", help="Administrator email", type=str, required=True)
  parser.add_argument(
      "--project",
  ...
  for i, user in enumerate(users, 1):
    ...
    if i % num_processes == process_number:
      logger.debug("Process #%s - User %d/%d: %s", process_number, i,
                   total_users, user)
      rows.extend(get_denormalized_scopes_for_user(directory_client, user))
...
def analyze():
  ...
  while processes:
    ...
    if message_type == "DATA":
      ...
      logger.error("Unknown message %s from process", message["message_type"])
      exit(1)
