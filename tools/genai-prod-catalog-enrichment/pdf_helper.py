...
def image_info_for_page(pdf_file,images_path,page,page_index,gcs_bucket=None):
    ...
    for i_no,image_info in enumerate(images_info,start=1):
        try:
            ...
            if ((bbox[0] < xmin or bbox[0] > xmax) and (bbox[2] < xmin or bbox[2] > xmax)) or ((bbox[1] < ymin or bbox[1] > ymax) and (bbox[3] < ymin or bbox[3] > ymax)):
                continue
            # print(bbox)
            ...
            if not(str(image_ext.lower()) == "jpeg" or str(image_ext.lower()) == "jpg" or str(image_ext.lower()) == "png"):
                continue
            i=i+1
            ...
            if gcs_bucket:
                ...
                with open(image_name , 'wb') as image_file:
                    image_file.write(image_bytes)
            image = {"xref": image_xref, "id": image_id, "image_url": url, "bbox": (bbox[0], bbox[1], bbox[2], bbox[3])}
            # print(image_name)
            # print(image["bbox"])
            images.append(image)
...
def get_info_for_page(pdf_file, page, page_index, images_path, gcs_bucket=None):
    text_info = {}
    image_info = []
    image_info = image_info_for_page(pdf_file, images_path, page, page_index, gcs_bucket)
    ...
    if filename.endswith('.pdf'):
        ...
        for i, page in enumerate(pdf_file):
            if page.get_text():
                ...
                text = False
                print(f"[ERROR]: Page {i+1} of {filename} PDF is a scanned page!")
                break
        return text
    else:
...
def get_info_for_pdf(filename, images_path, input_gcs_bucket=None, image_gcs_bucket=None):
    if input_gcs_bucket:
        bucket_object = storage.Client().bucket(input_gcs_bucket)
        blob = bucket_object.blob(filename)
        pdf_file = fitz.open("pdf", blob.download_as_bytes())
    ...
    for page_index, page in enumerate(pdf_file, start=1):
        page_info = get_info_for_page(pdf_file, page, page_index, images_path,image_gcs_bucket)
...
def parse_pdf(filename, input_gcs_bucket, output_gcs_bucket):
    ...
    pdf_json = get_info_for_pdf(filename,images_path,input_gcs_bucket,output_gcs_bucket)
