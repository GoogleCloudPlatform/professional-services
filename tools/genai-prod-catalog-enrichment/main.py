...
def download_blob(bucket_name, source_blob_name, destination_file_name, prefix, csv_folder):
    ...
    print(
        "[INFO]: Downloaded storage object {} from bucket {} to local file {}.".format(
...
def check_pdf_type(filename,input_gcs_bucket=None):
    ...
    for page_no, page in enumerate(pdf_file,start = 1):
        page_wise_details = {"Scanned":False,"Images":False,"Tables":False,"Total Images":0}
    ...
    return {"Scanned":Scanned, "Tables":Tables,"Images":Images,"Total_Images":Total_Images,"pages":pages}
...
def end_to_end_pipeline(input_pdf_uri, output_gcs_bucket, project_id):
    ...
    try:
        ...
        print(f"[INFO]: CSV data fetched successfully in {end - start} seconds")
    except Exception as e:
        print(f"[ERROR]: Error during fetching data from CSV - {e}")
        return None
        ...
        if not pdf_type["Scanned"]:
            ...
            if company_text:
                ...
                if "company_details" in company_details:
                    json_output_response["company_details"] = company_details["company_details"]
            ...
            print(f"[INFO]: Company details extraction time: {end - start} seconds")
            for page_no, page in enumerate(pdf_json['pages']):
                try:
                    ...
                    print(f"[INFO]: FAQ, ISQ generation time for page {page_no + 1}: {end - start} seconds")
                    ...
                    if len(products) > 0:
                        try:
                            ...
                            print(f"[INFO]: Product tag generation time for page {page_no + 1}: {end - start} seconds")
                        except Exception as e:
                            print(f"[ERROR]: Error during generation of product tags - {str(traceback.format_exc())}")
                            error_ws2_msg = error_ws2_msg + "\n" + f'product tags generation failed - {str(e)}'
                            product_tags = {}
                        try:
                            start = time.time()
                            ...
                            print(
                                f"[INFO]: Product category generation time for page {page_no + 1}: {end - start} seconds")
                        except Exception as e:
                            print(
                                f"[ERROR]: Error during generation of product category - {str(traceback.format_exc())}")
                            error_ws2_msg = error_ws2_msg + "\n" + f'product category generation failed - {str(e)}'
                            product_category = {}

                        try:
                            start = time.time()
                            image_json = generate_images_json(page, products, product_tags, output_gcs_bucket)
                            end = time.time()
                            print(f"[INFO]: Image JSON generation time for page {page_no + 1}: {end - start} seconds")
                        except Exception as e:
                            print(
                                f"[ERROR]: Error during generation of product category - {str(traceback.format_exc())}")
                            error_ws2_msg = error_ws2_msg + "\n" + f'Product image json generation failed - {str(e)}'
                            image_json = {}
                    else:
                        product_tags = {}
                        product_category = {}
                    ...
                    start = time.time()
                    json_output_response = update_output_json(json_output_response, faq_json, isq_json, image_json,
                                                              product_category, product_tags, error_ws2_msg)
                    end = time.time()
                    print(f"[INFO]: Output for page {page_no + 1}: updated in {end - start} seconds")
                except Exception as e:
                    print(f"[ERROR]: Error during generating final json - {str(traceback.format_exc())}")

            final_json_path = f"{name_initials}_ouput.json"
            start = time.time()
            create_json_from_dict(final_json_path, json_output_response, output_gcs_bucket)
            end = time.time()
            print(f"[INFO]: Final output JSON uploaded to GCS in {end - start} seconds")
        else:
