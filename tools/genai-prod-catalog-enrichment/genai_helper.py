...
def get_blocked_response_template():
    blocked_response = {
        "response_error": {
            "is_blocked": True,
            "safety_attributes": {},
            "message": "The response is blocked because the input or response potentially violates Google’s policies. Try rephrasing the prompt or adjusting the parameter settings."
        }
    }
...
def get_prompt(context, task, error="", product_name=""):
    ...
    faq_json_format = """{
        "catalogue_faqs": [
            {"question": "What is the size of the paper napkin produced by this machine?", "answer": "The paper napkin produced by this machine is 30 X 30 cm."},
            {"question": "What is the speed of this machine?", "answer": "This machine can produce 2,50,000 pieces in 8 hours."},
            {"question": "How many colors can this machine print?", "answer": "This machine can print up to 2 colors."},
            {"question": "What is the weight of this machine?", "answer": "This machine weighs approximately 2500 kgs."},
            {"question": "How many people are required to operate this machine?", "answer": "This machine requires one operator and one helper to operate."},
            {"question": "What type of raw material does this machine use?", "answer": "This machine uses tissue paper with a gsm of 12 to 30."}
        ]
    }"""
        ...
        "specifications": [
            {
                "SINGLE SIZE PAPER NAPKIN MACHINE": {
                    ...
                    "Motor": [
                        "3hp motor with variable AC drive with VDF (Variable frequency drive)",
                        "1hp motor (AC)"
                    ],
    ...
    malformed_json = """
    {
        ...
        'specifications': [
            {'Acrylic Body Rotameter': {
                ...
                'Flow Ranges': '2 to 60,000 LPH of water at ambient temperature and 0.1 to 750 Nm/hr of air at NTP',
    ...
    {
        "catalogue_faqs": [
            {"question": "What is the purpose of this diagram?", "answer": "This diagram shows the piping and wiring schematic for an AO Smith heat pump water heater."},
            {"question": "What are the different components shown in the diagram?", "answer": "The diagram shows the following components:
            ...
            8)    FFL note"},
            {"question": "What are the different steps involved in the operation of this system?", "answer": "The steps involved in the operation of this system are as follows:
            ...
            3)    The hot water is then distributed to the rooms through the hot water to rooms pipes.
            4)    The return water from the rooms is then sent back to the heat pump through the return water from rooms pipes.
            5)    The process repeats itself."},
            {"question": "What are some of the important safety precautions that should be taken when working on this system?", "answer": "Some of the important safety precautions that should be taken when working on this system include:
            1)    Always turn off the power to the system before working on it.
            2)    Be sure to use proper safety equipment, such as gloves and eye protection.
            3)    Never work on the system while it is hot.
            4)    Be aware of the location of all of the components in the system.
            5)    If you are unsure about anything, always consult a qualified professional."}
        ]
    }
    """
    ...
    "answer": "This diagram shows the piping and wiring schematic for an AO Smith heat pump water heater."},
    {"question": "What are the different components shown in the diagram?",
    "answer": "The diagram shows the following components:\n1) Hot water to rooms\n2) Tank temp sensor\n3) Hot water outlet\n4) Flow switch\n5) Vibration pads\n6) Return water from rooms\n7) Cold water inlet to heat pump\n8) FFL note"},
    {"question": "What are the different steps involved in the operation of this system?",
    "answer": "The steps involved in the operation of this system are as follows:\n1) Cold water enters the heat pump through the cold water inlet.\n2) The heat pump heats the water and sends it to the hot water tank.\n3) The hot water is then distributed to the rooms through the hot water to rooms pipes.\n4) The return water from the rooms is then sent back to the heat pump through the return water from rooms pipes.\n5) The process repeats itself."},
    {"question": "What are some of the important safety precautions that should be taken when working on this system?",
    "answer": "Some of the important safety precautions that should be taken when working on this system include:\n1) Always turn off the power to the system before working on it.\n2) Be sure to use proper safety equipment, such as gloves and eye protection.\n3) Never work on the system while it is hot.\n4) Be aware of the location of all of the components in the system.\n5) If you are unsure about anything, always consult a qualified professional."}]}
    """
    ...
    faq_prompt = f"""Generate a list of frequently asked questions (FAQ) based only on the provided input.

    Extract the key points, common queries, and important details to create a concise and informative set of questions and answers that would provide clarity on this subject for readers.

    Return the output in JSON format.

    input: 02 SINGLE SIZE PAPER NAPKIN MACHINE Size: 30 X 30 CM No of Printing : As Per Requirement Embossing Unit: As Per Requirement Motor: 1) 3hp motor with variable AC drive with VDF (Variable frequency drive) 2) 1hp motor (AC) Speed: 2,50,000 PIECES / 8 HOURS Weight: 2500 kgs (approx) Man Power: One operator & One helper Raw Material: Tissue paper 12 to 30 gsm Counting: digital Counting Band saw cutting with mauling sharping system PRICE WITHOUT PRINT : `4,50,000 1 COLOUR PRINT : `5,75,000 2 COLOUR PRINT : `6,25,000
    ...
    check_specs_prompt = f"""For the following text, examine if it contains a product description, specifications or features. If found, return a boolean response True. If not found, return a boolean response False.

    {context}
    """

    product_specs_prompt = f"""Convert the following text into a product specifications JSON containing \"product_name\" and other \"specifications\". Also, add a \"confidence_score\" to the end of the JSON.
    ...
    company_details_prompt = f"""Convert the following text into a JSON containing company details. Ensure that the details extracted are based solely on the content of the following text and are as accurate as possible.

    input: CONTACT US COMPLETE MACHINE GLOBAL CONVERSION MACHINES SALES TEAM MARKETING TEAM SERVICE TEAM + 91 958 215 2344 + 91 874 482 8924 + 91 888 291 3467 Email : globalconversionmachines@gmail.com Follow us on @ globalcmachines @ globalconversionmachines Global Conversion Machines
    ...
    fix_json_prompt = f"""Fix the error/malformation in the following JSON and ensure that you only return a valid JSON.
    ...
    tags_and_label_prompt = f"""Suggest some tags and a category for the given product name and description and convert it into JSON format. Ensure that the suggestions are based solely on the content of the text and are as accurate as possible.

    input: Product Name: Non Woven Bag Making Machine
    Product Description: 07 NON WOVEN BAG MAKING MACHINE Technical Speciﬁcation Fabric Paper Tube Diameter: 2.75 -3 Inches Max Speed: 20 -120 Bags /min Bag Width: 3.9-32 Inches Bag Height: 7.75-24mm Bottom Insert Size: 1.20-3.25 Inches Side Folding Size: 1.20-3.25 Inches Bag Thickness: 30-120g Power Supplier: 220v/380v Power: 12kw 15kw Overall Dimension (L*w*h): 7600*1900*2100mm Weight: 2200 Kg Air Compressor: 0.6-1.0 Mpa Unwinding Method: Magnetic Power Tension Control Unwinding Diameter: 1000 Mm Max.width Of Unwinding: 1300 Mm Correction Device: Photoelectric Epc System An Synchronous Rectiﬁcation Motor 110w PRICE PRICE: ` 15,95,000
    output: {non_woven_bag}

    input: Product Name: Toilet Roll Machine
    Product Description: 04 TOILET ROLL MACHINE PRODUCT PRICE PRODUCTION TYPE 1 4500 Rolls / 8 Hours Toilet Roll ` 4,75,000 TYPE 2 9000 Rolls / 8 Hours Toilet Roll, ` 5,25,000 Kitchen Rolls , Non Woven Cleaning Rolls, Hospital Bed Tissue Rolls. TYPE 3 17,000 Rolls / 8 Hours Toilet Roll, `11,50,000 Kitchen Rolls, Non Woven Cleaning Rolls, Hospital Bed Tissue Rolls. TYPE 4 Any Customized Production Capacity
    ...
    fix_faq_json_prompt = f"""Fix the error/malformation in the following JSON and ensure that you only return a valid JSON.
...
def fix_json(error, context, project):
    try:
        ...
        if not fix_json_response.is_blocked:
            ...
            response["response_error"]["safety_attributes"] = fix_json_response.safety_attributes
            return response

        return response

    except SyntaxError as e:
        print(f"[ERROR]: SyntaxError during fixing JSON. The LLM may have again returned a malformed JSON! \n{e}\n")
    ...
    try:
        ...
        for product in products:
            ...
            if not tags_and_labels_response.is_blocked:
                ...
                response = get_blocked_response_template()
                response["response_error"]["safety_attributes"] = tags_and_labels_response.safety_attributes
                return response
...
def generate_isqs(context, project):
    try:
        ...
        if not product_isqs.is_blocked:
            ...
            print(f"[WARNING]: ISQ Generation Response blocked by LLM: {product_isqs.safety_attributes}")
            isq_response = get_blocked_response_template()
            isq_response["response_error"]["safety_attributes"] = product_isqs.safety_attributes
            return isq_response

        return isq_response

    except SyntaxError as e:
        print(f"[ERROR]: SyntaxError during ISQ generation. The LLM may have returned a malformed JSON! \n{e}\n")
...
def generate_faqs(context, project):
    try:
        ...
        if not faq_response.is_blocked:
            response = ast.literal_eval(faq_response.text.strip().replace('null', 'None'))
        else:
            print(f"[WARNING]: FAQ Generation Response Blocked by LLM. {faq_response.safety_attributes}")
            response = get_blocked_response_template()
            response["response_error"]["safety_attributes"] = faq_response.safety_attributes
            return response

        return response

    except SyntaxError as e:
        print(f"[ERROR]: SyntaxError during FAQ generation. The LLM may have returned a malformed JSON! \n{e}\n")
...
def generate_company_details(company_text, project):
    try:
        ...
        company_details_response = get_model_response(company_details_prompt, project)
...
def fix_faq_json(error, context):
    try:
        ...
        print(f"[ERROR]: SyntaxError during fixing FAQ JSON. The LLM may have again returned a malformed JSON! \n{e}\n")
        print(fix_json_response.text.strip())
        response = get_failed_faq_template()
        response["catalogue_faqs"][0]["response_error"] = f"{e}"
...
Prodcuct A is a  bench . It is of white color. I am having a good day. I need a toilet roll making machine like product B.
...
Example Extracted Text
Toilet roll making machine can make toilet rolls easily. Its weight is 2kg. Contly medicine tablets can cure liver diseases. It has no side effects.
...
Example Extracted Text
Toilet roll making machine can make toilet rolls easily. It's weight is 2kg. Contly medicine tablets can cure liver diseases. It has no side effects.
...
Specific caption guesses of image might be very specific but can be sometimes wrong.
Specific caption guesses contain 3 values, with 1st one has higher chances of being correct and 3rd one has comparatively lower chances of being correct.
...
Output should be in json format with product as key and mapped image as value. If no image can be mapped for a product, then simply keep its value as empty.
...
To better understand products, we also have mapped tags for some of the products, might not be there for all products.
Compare the main caption with product tags for more acccurate product image mapping.
Check product tags properly, so that you dont confuse similar name object with actual products.
example: image of toilet rolls are not to be confused or mapped with toilet roll making machine
