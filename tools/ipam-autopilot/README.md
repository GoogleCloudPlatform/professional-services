...
You will need to have PGP set up the provider to build.
...
1. Deploy the provider structure `terraform init` and `terraform apply`
1. Deploy the IPAM Autopilot backend `terraform init` and `terraform apply`


Please be aware, this approach towards Terraform provider registry is rather brittle.
