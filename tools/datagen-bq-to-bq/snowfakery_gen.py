...
def generate_receipes(table_attributes,gemini_model):
    ...
    for table_name in config_vars.user_counts:
        ...
        if attributes is None:
            print(f"Warning: Table '{table_name}' not found in table_attributes. Skipping recipe generation.")
            continue
      ...
      # Accessing the attributes for a specific table
        ...
        Recipe_Prompt=prompts_collection.create_receipe_prompt(prompts_collection.fake_function_list,schema)
        file_records = Part.from_uri(staging_gcs_path, mime_type="text/csv")
        yaml_output = gemini_model.generate_content([Recipe_Prompt, "Sample Records:", file_records]).text
        chat_yaml_corection = gemini_model.start_chat()
        final_yaml_output=chat_yaml_corection.send_message(prompts_collection.yaml_correction_prompt + """ **Input:** """ + yaml_output).text
...
def generate_output_data(
        ...
        relationship_output= gemini_model.generate_content([prompts_collection.Relationship_builder,combined_schema])
        print(relationship_output.text)
        combined_yaml = gemini_model.generate_content([prompts_collection.Relationship_Prompt,relationship_output.text,combined_yaml])
        combined_yaml = gemini_model.generate_content([prompts_collection.Correct_Object_Name_Prompt,relationship_output.text,combined_yaml.text])
