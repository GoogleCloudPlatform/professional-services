...
def export_bigquery_table_to_bq_gcs_staging(
                ...
                staging_path_bigquery,input_gcs_path # Pass the batch-specific path
            ):
  """
  ...
  if config_vars.SOURCE_TYPE == "BigQuery":
    ...
    for table_name in input_bq_table_names.split(','):
        ...
        print(f"Exported {num_rows if num_rows else 'all'} rows from {table_name} to {destination_uri}")
    ...
    for table_name in table_names:
        ...
        input_gcs_path[simple_table_name] = f"{staging_path_bigquery}/{simple_table_name}.csv"
...
def import_gcs_to_bigquery(table_attributes):
    ...
    for table_name in config_vars.user_counts:
        ...
        source_table_id = config_vars.source_bq_project_id+"."+config_vars.source_bq_dataset+"."+table_name
