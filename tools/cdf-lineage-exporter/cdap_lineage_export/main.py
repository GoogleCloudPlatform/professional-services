...
def handle_http(request: flask.Request):
    ...
    datasets, streams = export_all_cdap_lineage_to_gcs(
        ...
        request_json.get("bucket"),
        request_json.get("apiEndpoint"),
        request_json.get("startTimestamp"),
...
class LineageScraper:
    ...
    def get_namespace_lineages(self):
        ...
        for namespace in namespaces:
            ...
            _get_lineage = partial(
                self.client.list_lineage,
            ...
            namespace.stream_lineages = [
                ...
                for stream in self.client.list_streams(namespace=namespace.name)
            ]
        return namespaces
...
def export_all_cdap_lineage_to_gcs(project: str,
                                   bucket_name: str,
                                   api_endpoint: str,
                                   start_ts: str,
                                   end_ts: str,
                                   bucket_project: str = None
                                  ) -> Tuple[storage.Blob, storage.Blob]:
    ...
    cdap_client = CDAPClient(api_endpoint)
    storage_client = storage.Client(project=bucket_project or project)
    ...
    for namespace in namespaces:
        if namespace.dataset_lineages:
            ...
            dataset_lineage_ndjson = "\n".join(
                map(json.dumps, namespace.dataset_lineages))
            datasets_blob.upload_from_string(dataset_lineage_ndjson)
...
def main():
    ...
    export_all_cdap_lineage_to_gcs(args.project,
                                   args.bucket,
                                   args.api_endpoint,
                                   args.start_ts,
                                   args.end_ts,
                                   bucket_project=args.bucket_project)
...
def parse_lineage_scraper_args() -> argparse.Namespace:
    ...
    parser.add_argument("--project",
                        ...
                        required=True,
                        dest="project")
    parser.add_argument("--bucket_project",
                        help="GCP Project ID for Storage bucket")
    parser.add_argument(
        "--api_endpoint",
        "-a",
