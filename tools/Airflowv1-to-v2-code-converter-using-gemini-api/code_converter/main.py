    ...
    for blob in blobs:
        if not blob.name.endswith('/'): # Skip "folders"
            if file_extension_filter:
                if any(blob.name.lower().endswith(ext) for ext in file_extension_filter):
...
def convert_code_with_vertexai(model, system_instruction, user_question_template, prompt_variables, code_content):
    ...
    try:
        response = model.generate_content(
            ...
            # generation_config={"temperature": 0.2} # Can add other configs
        )
        # For safety ratings
        # if response.prompt_feedback.block_reason:
        #     raise ValueError(f"Request blocked due to: {response.prompt_feedback.block_reason_message}")
        # if not response.candidates or not response.candidates[0].content.parts:
        #     raise ValueError("No content generated by the model or unexpected response structure.")

        converted_code = response.text
        print(f"Starting cleaning process for a code snippet") # Clarified log
        ...
        elif converted_code.startswith("```sql"): # Added sql example
            converted_code = converted_code[len("```sql"):]
        elif converted_code.startswith("```"): # Generic ``` removal
        ...
        print(f"Successfully converted code snippet")
...
def process_single_file(input_bucket_name, input_folder_prefix_in_bucket, full_input_file_gcs_path,
                        output_bucket_name, output_folder_prefix_in_bucket,
                        model, system_instruction, user_question_template, prompt_variables):
    """
    ...
    Args:
        ...
        full_input_file_gcs_path (str): The full GCS path of the input file (e.g., "dags/v1/team_a/dag_file.py").
        output_bucket_name (str): Name of the GCS bucket to write to.
        output_folder_prefix_in_bucket (str): The prefix path in the output bucket where converted files
                                              will be stored (e.g., "dags/v3_converted/").
    ...
    try:
        ...
        relative_path_to_input_prefix = os.path.relpath(full_input_file_gcs_path, start=input_folder_prefix_in_bucket or ".")
        final_output_file_gcs_path = os.path.join(output_folder_prefix_in_bucket, relative_path_to_input_prefix)
        ...
        #    final_output_file_gcs_path = base + new_extension_suffix + old_ext # Or just new_extension
        # else:
...
def main_handler(request):
    ...
    try:
        ...
        project_id = request_json.get("project_id", os.environ.get("GOOGLE_CLOUD_PROJECT"))
        ...
        input_folder_path = request_json.get("input_folder_path", "") # Default to root if not specified
        write_bucket_name = request_json.get("write_bucket_name")
        output_folder_path = request_json.get("output_folder_path") # Base path for output files
        ...
        if not isinstance(system_instruction_parts, (list, str)):
             return "Error: 'system_instruction' must be a string or a list of strings.", 400


        # --- Initialize Vertex AI ---
        print(f"Initializing Vertex AI for project '{project_id}' in location '{location}' with model '{model_id}'...")
        ...
        print(f"Listing files in gs://{read_bucket_name}/{list_prefix} with filter: {file_extension_filter}")
        input_files = list_gcs_files(read_bucket_name, list_prefix, file_extension_filter)
        if not input_files:
        ...
        print(f"Processing summary: {json.dumps(summary, indent=2)}") # Pretty print summary
