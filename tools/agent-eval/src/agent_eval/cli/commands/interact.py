...
def interact(
    ...
    if raw_df.empty:
        ...
        console.print(
            "    [dim]Check that your golden dataset has questions and the agent is responding.[/]"
        )
        sys.exit(0)
    ...
    console.print(
        f"    [green]+[/] Saved [cyan]{len(records)}[/] enriched interaction{'s' if len(records) != 1 else ''}"
    )
