...
def _render_metric_review_guide(
    ...
    for name, defn in custom_metrics.items():
        if is_managed_entry(defn):
            ...
            scale = ("binary 0/1" if scores == ["0", "1"] else
                     f"scale {scores[0]}-{scores[-1]}" if scores else "?")
            tags = []
...
def _required_reference_fields(
    ...
    for name, defn in custom_metrics.items():
        ...
        for _placeholder, mapping in (defn.get("dataset_mapping") or
                                      {}).items():
...
def _verify_environment(auto_approve: bool = False) -> None:
    ...
    from agent_eval.cli.commands.setup import _adc_file_path
    from agent_eval.cli.commands.setup import _check_api_enabled as _setup_check_api
...
def _display_agent_analysis(analysis: dict[str, Any]) -> None:
    ...
    if tools:
        ...
        if tool_interactions_available(analysis):
            table.add_row("tool_interactions",
...
def _prompt_ai_metrics_multistep(
    ...
    metrics_path = (project_root / "tests" / "eval" / "metrics" /
...
def _load_existing_eval_data(
    agent_dir: Path,) -> tuple[list | None, list | None]:
...
def _metric_reference_fields(defn: dict) -> set[str]:
    ...
    for config in mapping.values():
        if not isinstance(config, dict):
            continue
        cols = [
            config.get("source_column", ""), *config.get("source_columns", [])
        ]
        for col in cols:
...
def _display_connection_map(
    ...
    if ref_metric_fields:
        ...
        for name, fields in sorted(ref_metric_fields.items()):
            field_str = (", ".join(sorted(fields)) if fields != {"*"} else
                         "any populated field (auto-resolved)")
            console.print(
