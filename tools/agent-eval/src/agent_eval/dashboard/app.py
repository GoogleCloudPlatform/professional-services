...
def _render_run_metadata(runs: list[RunInfo], baseline_id: str,
    ...
    def _run_row(run: RunInfo, label: str) -> str:
        commit = (run.git_info.get("commit", "")[:8]
                  if run.git_info.get("commit") else "n/a")
    ...
    with gr.Blocks(theme=gr.themes.Default(),
        ...
        with gr.Tab("Per-Question Drilldown"):
            pq_metric_dd = gr.Dropdown(
                choices=[
                    *all_llm_metrics, "tool_success_rate.tool_success_rate"
                ],
                label="Metric",
        ...
        def update_overview_table(baseline_id, compare_id):
            ...
            for key in all_keys:
                ...
                delta = (compute_delta(bv or 0, cv or 0, key)
                         if bv is not None and cv is not None else {})
                rows.append({
                    "Metric":
                        _display_name(key),
                    "Baseline":
            ...
            for key in all_llm:
                ...
                sr = b.llm_metrics.get(key, c.llm_metrics.get(key, {})).get(
                    "score_range", {})
                score_label = (f" ({sr.get('min', '?')}-{sr.get('max', '?')})"
                               if sr else "")
                rows.append({
                    "Metric":
