    ...
    if p.endswith(".jsonl"):
        ...
        if not Path(p).exists():
            raise FileNotFoundError(
                f"Dataset file not found at '{p}'") from None
        rows = read_dataset(p)
...
def filter_questions_by_metadata(
    ...
    for question in questions:
        ...
        for filter_key, filter_values in filters.items():
            ...
            if (filter_key not in metadata or
                    str(metadata[filter_key]) not in filter_values):
...
def parse_metadata_filters(
        filter_strings: list[str] | None) -> dict[str, list[str]]:
...
def parse_state_variables(
        state_var_strings: list[str] | None) -> dict[str, Any]:
