...
def compute_comparison(
    ...
    for key in sorted(all_llm_keys):
        ...
        if b_info is None:
            c_avg = (c_info.get("average", c_info)
                     if isinstance(c_info, dict) else c_info)
            new_metrics.append({"metric": key, "type": "llm", "current": c_avg})
            continue
        if c_info is None:
            b_avg = (b_info.get("average", b_info)
                     if isinstance(b_info, dict) else b_info)
            removed_metrics.append({
                "metric": key,
                "type": "llm",
                "baseline": b_avg
        ...
        score_range = (c_info if isinstance(c_info, dict) else {}).get(
            "score_range", {})
        range_width = (score_range.get("max", 5) -
                       score_range.get("min", 0) if score_range else None)
    ...
    git_diff = ""
    if (b_git.get("commit") and c_git.get("commit") and
...
def format_comparison_table(comparison: dict) -> str:
    ...
    for d in comparison["deltas"]:
        b_str = (f"{d['baseline']:.2f}"
                 if isinstance(d["baseline"], float) else str(d["baseline"]))
        c_str = (f"{d['current']:.2f}"
                 if isinstance(d["current"], float) else str(d["current"]))
        d_str = (f"{d['delta']:+.2f}"
                 if isinstance(d["delta"], float) else str(d["delta"]))
    ...
    def _process_log_row(self, row: pd.Series, index: int) -> LogEntry | None:
        ...
        try:
            ...
            if isinstance(latency_data, list) and latency_data:
                total_s = sum(
                    span.get("duration_seconds", 0) for span in latency_data if
                    isinstance(span, dict) and span.get("name") == "invocation")
                if total_s > 0:
...
| **Metadata** | {metadata_str} |"""
        ...
        fr = (entry["final_response"]
              if isinstance(entry["final_response"], str) else str(
                  entry["final_response"] or ""))
        final_response = fr[:500] + "..." if len(fr) > 500 else fr
        response_md = f"### Final Response Summary\n\n{final_response}\n"

        # Agent trajectory
        trajectory = (" → ".join(entry["trace_summary"])
                      if entry["trace_summary"] else "N/A")
...
"""
    ...
    def _auto_find_previous_run(self, results_dir: Path,
                                current_run_folder: Path) -> Path | None:
    ...
    async def generate_gemini_analysis(
        ...
        analysis_text = ""
        try:
            response = await client.aio.models.generate_content(model=model,
                                                                contents=prompt)
    ...
    async def run(self) -> dict | None:
        ...
        try:
            ...
            gemini_md = ((run_folder / "gemini_analysis.md").read_text() if
                         (run_folder / "gemini_analysis.md").exists() else None)
