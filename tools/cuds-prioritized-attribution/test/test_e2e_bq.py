...
class TestBQ:
    ...
    def get_output_results(test_case_no):
        ...
        TestBQ.set_env_var('billing_export_dataset_id', billing_export_dataset_id)

        commitment_table_name = TestBQ.get_env_var('TABLE_PREFIX') + test_case_no + "_commitments"
        TestBQ.set_env_var('commitment_table_name', commitment_table_name)

        billing_export_table_name = TestBQ.get_env_var('TABLE_PREFIX') + test_case_no + "_export"
        TestBQ.set_env_var('billing_export_table_name', billing_export_table_name)

        distribute_commitments_query = TestBQ.get_env_var('DISTRIBUTE_COMMITMENTS_QUERY')
        TestBQ.set_env_var('distribute_commitments_query', distribute_commitments_query)
        ...
        distribute_commitment_table = TestBQ.get_env_var('DISTRIBUTE_COMMITMENT_TABLE')
        distribute_commitment_table = distribute_commitment_table + test_case_no
        TestBQ.set_env_var('distribute_commitment_table', distribute_commitment_table)
        ...
        results = TestBQ.select_from_table(corrected_dataset_id, output_table_name)
        return results
    ...
    def fill_up_map_from_result(results, credit_type):
        ...
        for row in results:
            ...
            if (credit_type in sku_id) and ('CREDIT' in sku_id):
                ...
                for item in credits:
                    ...
                    project_credit_all[project_credit_name] += output_credit_amount
        ...
        project_credit_data = (total_credit, project_credit_all, project_cost_all)
    ...
    def get_output_map(test_case_no, credit_type):
        ...
        project_credit_data = TestBQ.fill_up_map_from_result(results, credit_type)
        total_credit, project_credit_all, project_cost_all = project_credit_data
        output_project_credit_data = (total_credit, project_credit_all, project_cost_all)
    ...
    def get_expected_output_map(test_case_no, credit_type):
        ...
        table_name = TestBQ.get_env_var('TABLE_PREFIX') + test_case_no + "_expected"
        results = TestBQ.select_from_table(dataset_id, table_name)
        project_credit_data = TestBQ.fill_up_map_from_result(results, credit_type)
        total_credit, project_credit_all, project_cost_all = project_credit_data
        expected_project_credit_data = (total_credit, project_credit_all, project_cost_all)
    ...
    def test_project_credit_cost_sum(self, testcasename):
        ...
        output_project_credit_data = TestBQ.get_output_map(testcasename, credit_type)
        total_credit, project_credit_all, project_cost_all = output_project_credit_data
        expected_project_credit_data = TestBQ.get_expected_output_map(testcasename, credit_type)
