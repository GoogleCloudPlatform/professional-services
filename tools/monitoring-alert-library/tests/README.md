...
This framework uses `pytest` to execute and validate Google Cloud (`gcloud`) commands based on test definitions written in YAML files. It's designed to test interactions with Google Cloud resources, in the context of detecting trigger of monitoring alerts.
...
export PREFIX="monitoring-alerts-library"
export CHANNEL_TESTING_TOPIC="monitoring-alerts-testing"
export CHANNEL_TESTING_SUBSCRIPTION="monitoring-alerts-testing"
```
...
```bash
gcloud pubsub topics create $CHANNEL_TESTING_TOPIC --project=$PROJECT_ID
```
...
gcloud beta monitoring channels create \
    --project=$PROJECT_ID \
    --type=pubsub \
    --display-name="Pub/Sub Monitoring Testing Alerts Channel" \
    --channel-labels=topic="projects/$PROJECT_ID/topics/$CHANNEL_TESTING_TOPIC"

Created notification channel [projects/dbs-validator-kcc-29ae/notificationChannels/3222290270956763642].
```

```bash
gcloud pubsub subscriptions create $CHANNEL_TESTING_SUBSCRIPTION \
    --topic=$CHANNEL_TESTING_TOPIC \
    --project=$PROJECT_ID \
    --ack-deadline=30 \
    --message-retention-duration=1d
```

4. ** Configure monitoring alert with this notification channel**

Edit the values.yaml file as below to configure notification channel
...
notification_channels:
  - projects/dbs-validator-kcc-29ae/notificationChannels/8590798822156370549
alerts:
  projectOwnershipChange: {}
```

5. **Deploy the monitoring alert
