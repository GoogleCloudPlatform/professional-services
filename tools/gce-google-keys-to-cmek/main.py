...
def main():
    parser = argparse.ArgumentParser(
        description=
        'Convert disks attached to a GCE instance from Google-managed encryption keys to customer-managed encryption keys.'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        'Name of the key ring containing the key to encrypt the disks. Must be in the same zone as the instance.'
    )
    parser.add_argument(
        '--key-name',
        required=True,
        ...
        type=str,
        help=
        'Name of the key to encrypt the disks. Must be in the same zone as the instance.'
    )
...
def migrate_instance_to_cmek(project, zone, instance, key_ring, key_name,
    ...
    for source_disk in disks:
        ...
        if 'diskEncryptionKey' in source_disk:
            logging.info('Skipping %s, already encrypyed with %s',
                         existing_disk_name, source_disk['diskEncryptionKey'])
            continue
        ...
        create_disk(compute, project, region, zone, snapshot_name,
                    new_disk_name, disk_type, key_name)
