...
class WriteAttachmentToGCS(beam.DoFn):
  ...
  def getattachment(self, email, message_id, attachement_id):
    ...
    try:
      ...
      att = gmail.users().messages() \
        .attachments().get(userId=email,
  ...
  def process(self, element):
    ...
    for i in range(len(e['payload']['parts'])):
      if 'attachmentId' in e['payload']['parts'][i]['body']:
        attachment_id = e['payload']['parts'][i]['body']['attachmentId']
        ...
        for j in range(len(e['payload']['headers'])):
          if e['payload']['headers'][j]['name'] == 'To':
            ...
            em = em[start:end]
            att_text = self.getattachment(em, message_id, attachment_id)
            break
        break
...
def run(input_topic, output_path, pipeline_argsrun=None):
  pipeline_options = PipelineOptions(
    pipeline_argsrun, streaming=True, save_main_session=True
  )
...
if __name__ == '__main__':  # noqa
  ...
  parser.add_argument(
    ...
    type=float,
    default=1.0,
    help='Output files window size in number of minutes.',
  )
  parser.add_argument(
    '--output_path',
