...
class Recommendation(object):
  ...
  def __repr__(self):
    return repr((self.state, self.member, self.member_type, self.remove_role,
  ...
  def extract_recommendation(self, data):
    ...
    for op_grps in data.get("content", {}).get("operationGroups", []):
      for op in op_grps["operations"]:
        if op["action"] == "remove":
          self.member.add(op["pathFilters"]["/iamPolicy/bindings/*/members/*"])
          self.resource.add(op["resource"])
          self.remove_role.add(op["pathFilters"]["/iamPolicy/bindings/*/role"])
        elif op["action"] == "add":
          self.resource.add(op["resource"])
          self.add_roles.add(op["pathFilters"]["/iamPolicy/bindings/*/role"])
          self.member.add(op["value"])
        else:
  ...
  def check_integrity(self):
    ...
    assert len(
        self.member
    ) == 1, "there should be exactly one member. member : " + str(self.member)
    assert len(
        self.remove_role
    ) == 1, "there should be exactly one removed role. remove_role: " + str(
        self.remove_role)
    assert len(self.resource
  ...
  def update_data(self):
    ...
    self.member = self.member.pop()
    self.member_type = self.member.split(":")[0]
...
def get_recommendations(project_id, recommender, state, credentials):
  ...
  request = recommender.projects().locations().recommenders().recommendations(
  ).list(
      parent=parent, fields=",".join(fields))
...
def diff_between_policies(old_policy, new_policy):
  ...
  for b in old_policy["bindings"]:
    ...
    for member in b["members"]:
      old_bindings[member].add(b["role"])
  new_bindings = collections.defaultdict(set)
  for b in new_policy["bindings"]:
    if "condition" in b:
      continue
    for member in b["members"]:
      new_bindings[member].add(b["role"])
  all_members = {*old_bindings.keys(), *new_bindings.keys()}
  entries = []
  for member in sorted(all_members):
    new_roles = new_bindings[member]
    old_roles = old_bindings[member]
...
def remove_role_from_policy(policy, recommendation):
  ...
  for binding in policy["bindings"]:
    if binding["role"] not in recommendation["role_recommended_to_be_removed"]:
      continue
    if "condition" in binding:
      continue
    try:
      is_acted_recommendation = True
      binding["members"].remove(recommendation["member"])
      recommendation["role_recommended_to_be_removed"].remove(binding["role"])
      acted_and_succeeded = True
    except:
      logging.error("`%s` does not have `role:%s`.", recommendation["member"],
...
def add_roles_in_policy(policy, recommendation):
  ...
  for binding in policy["bindings"]:
    ...
    if "condition" in binding:
      continue
    binding["members"].append(recommendation["member"])
