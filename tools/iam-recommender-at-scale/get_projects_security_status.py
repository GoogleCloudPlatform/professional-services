...
def accounts_can_made_safe(project_id, state, recommendations):
  ...
  member_types = ["serviceAccount", "user", "group"]
  columns = [
      "Number of recommendations on {}".format(member_type)
  ...
  for recommendation in recommendations:
    if recommendation.state != state:
      continue
    safe_accounts[recommendation.member_type].add(recommendation.member)
  return {
      "project_id": project_id,
      "stats": {
          column: len(safe_accounts[member_type])
          for column, member_type in zip(columns, member_types)
      }
  }
...
def main():
  parser = argparse.ArgumentParser(
      description="Find recommendation status of projects from your organization."
  )
  parser.add_argument(
      "--organization",
      required=True,
  ...
  logging.basicConfig(
      format="%(levelname)s[%(asctime)s]:%(message)s", level="INFO")
