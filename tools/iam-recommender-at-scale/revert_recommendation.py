...
def update_policy_to_revert_recommendations(policy, recommendations):
  ...
  for recommendation in recommendations["recommendations"]:
    ...
    is_success_removed = common.remove_role_from_policy(new_policy,
                                                        recommendation)
...
def update_recommendation_status_after_revert(client, recommendations,
  ...
  successful_revert_recommendation = [
      r for r, status in zip(recommendations["recommendations"], success_status)
      if status
  ]
  metadata = {
      "reverted":
  ...
  f = functools.partial(
      common.update_recommendation_status,
...
def main():
  ...
  parser.add_argument(
      ...
      required=True,
      type=str,
      help="Enter the location of file containing the recommendations to be reverted."
  )
  parser.add_argument(
      "--to_json",
      type=str,
