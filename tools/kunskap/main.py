...
def get_last_query_time(datastore_client):
    """Fetches the last time a transformation query was executed from Datastore.
...
def delete_partitions(partition_list, bq_client):
    ...
    for partition in partition_list:
        ...
        if partition_exists(bq_client, table_name):
            log_message = Template('Deleting partition $table')
            logging.info(log_message.safe_substitute(table=table_name))
...
def execute_transformation_query(date_list, bq_client):
    ...
    if date_list:
        ...
        table_name = Template(
            '$project_id:$output_dataset.$table_name').safe_substitute(
                project_id=config.billing_project_id,
