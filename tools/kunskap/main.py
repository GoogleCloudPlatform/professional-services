...
def get_usage_dates(partition_ids):
  ...
  sql = (
    ...
    'FROM `' + dataset_id + '.' + billing_table_name + '` '
...
def get_changed_partitions():
  ...
  if last_query_time:
    table_name = dataset_id + '.' + billing_table_name + '$__PARTITIONS_SUMMARY__'
...
def execute_transformation_query(dates_to_update):
  ...
  table_name = bq_client.project + '.' + dataset_id + '.' + output_table_name
  table_list = [table.full_table_id for table in list(bq_client.list_tables(dataset_id))]
  if table_name in table_list:
    table_ref = bq_client.dataset(dataset_id).table(output_table_name)
  else:
    table = bq_client.dataset(dataset_id).table(output_table_name)
