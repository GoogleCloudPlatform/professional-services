resource "google_dataproc_cluster" "dataproc-backend" {
  ...
  cluster_config {
    ...
    software_config {
      ...
      override_properties = {
        "hive:hive.metastore.warehouse.dir" = "gs://${var.bucket_name}/datasets"
      }
    }

    gce_cluster_config {
      metadata {
        hive-metastore-instance = "${var.project}:${var.region}:${var.cloud_sql_instance_name}"
      }
