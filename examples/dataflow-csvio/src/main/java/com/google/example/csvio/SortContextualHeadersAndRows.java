...
/**
 * A {@link PTransform} responsible for bundling the raw {@link ContextualTextIO#read()}
 * {@link Row} {@link PCollection} output into the corresponding CSV headers and rows.
 */
@AutoValue
abstract class SortContextualHeadersAndRows
    ...
    /**
     * The result of processing the raw {@link ContextualTextIO#read()}
     * {@link Row} {@link PCollection} output into the corresponding CSV headers and rows.
     */
    ...
    /**
     * A {@link PTransform} responsible for bundling the raw {@link ContextualTextIO#read()}
     * {@link Row} {@link PCollection} output into the corresponding CSV headers and rows in the
     * setting where a fixed header position is expected.
     */
     ...
     * A {@link PTransform} responsible for bundling the raw {@link ContextualTextIO#read()}
     * {@link Row} {@link PCollection} output into the corresponding CSV headers and rows in the
     * setting where a regular expression header matching is expected.
     */
    static class SortMatchedHeaderAndRows extends PTransform<PCollection<Row>, Result> {
        ...
        public Result expand(PCollection<Row> input) {
            ...
            PCollectionView<Map<String, Long>> mappedHeaderPositionView = resourceIdHeaderPosition.apply(View.asMap());
            PCollectionTuple pct = input.apply(
                    TAG_BASE + "/" + SortMappedFixedPositionHeadersAndRowsFn.class.getSimpleName(),
                    ParDo.of(new SortMappedFixedPositionHeadersAndRowsFn(mappedHeaderPositionView)
            ).withSideInput("", mappedHeaderPositionView).withOutputTags(HEADERS, TupleTagList.of(ROWS)));
     ...
     * A {@link PTransform} responsible for bundling the raw {@link ContextualTextIO#read()}
     * {@link Row} {@link PCollection} output into the corresponding CSV headers and rows in the
     * setting where the header is expected at the first non-empty line of the CSV file.
     */
    ...
    /**
     * The {@link DoFn} responsible for sorting a fixed header position given a with the subsequent CSV
     * records.  The header is mapped to the exact position expected.
     */
    static class SortMappedFixedPositionHeadersAndRowsFn extends DoFn<Row, Row> {
        ...
        SortMappedFixedPositionHeadersAndRowsFn(PCollectionView<Map<String, Long>> mappedResourceIdHeaderPositionView) {
        ...
        public void process(ProcessContext ctx) {
            ...
            Map<String, Long> resourceIdHeaderPosition = ctx.sideInput(this.mappedResourceIdHeaderPositionView);
    ...
    /**
     * Determines whether the CSV record {@link ContextualTextIO#read()} output is a
     * non-empty line.
     */
    ...
    /**
     * Converts the CSV record {@link ContextualTextIO#read()} output to a {@link KV} pairing
     * the {@link RecordWithMetadata#RECORD_NUM} with the {@link RecordWithMetadata#RESOURCE_ID}.
     */
    ...
    /**
     * A convenience wrapper for the {@link Row} with the expected
     * {@link RecordWithMetadata#getSchema()}.
     */
