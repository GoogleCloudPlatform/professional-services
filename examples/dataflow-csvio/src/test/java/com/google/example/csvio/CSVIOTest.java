...
class CSVIOTest {
  ...
  private static final List<CSVRecord> HEADER1_EXPECTED_ROWS = Arrays.asList(
  ...
  private static final List<CSVRecord> HEADER2_EXPECTED_ROWS = Arrays.asList(
  ...
  void read() {
    ...
    CSVIOReadConfiguration configuration = CSVIOReadConfiguration.builder()
        .setFilePattern(FILE_PATTERN)
        .build();

    CSVIO.Read.Result result = p.apply(
        CSVIO.read()
    ...
    for (Entry<String, List<CSVRecord>> expected : EXPECTED.entrySet()) {
      PCollection<CSVRecord> actual = result.getSuccess()
          .apply(Filter.by(new FilterByHeader(expected.getKey())));
