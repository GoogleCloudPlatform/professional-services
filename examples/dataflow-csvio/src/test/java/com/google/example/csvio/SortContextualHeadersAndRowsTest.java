...
class SortContextualHeadersAndRowsTest {
    ...
    void testSortFixedPositionHeadersAndRowsFn() {
        for (TestCase caze : CASES) {
            ...
            PCollectionTuple pct = input.apply(ParDo.of(
                    ...
                    SortContextualHeadersAndRows.HEADERS, TupleTagList.of(SortContextualHeadersAndRows.ROWS)));
    ...
    void testFindMatchedHeaderFn() {
        for (TestCase caze : CASES) {
            Pipeline p = Pipeline.create();
            PCollection<Row> input = readCSV(p, caze.getResourceNameAbsolutePath());
            PCollectionView<String> headerMatchRegexView = p.apply(Create.of(EXPECT_HEADER)).apply(View.asSingleton());
            String resourceId = caze.getResourceNameAbsolutePath();
            Long expectedHeaderPosition = EXPECT_HEADER_POSITION.get(resourceId);
            List<KV<String, Long>> expected = Collections.singletonList(KV.of(resourceId, expectedHeaderPosition));
    ...
    private static TestCase testCase(CSVIOReadConfiguration.Builder configuration, String resourceName) {
    ...
    private static String absolutePath(String resourceName) {
        return Objects.requireNonNull(SortContextualHeadersAndRowsTest.class.getResource(resourceName)).getFile();
    }
}