...
class Verbose(DoFn):
    def process(self, element):
        ...
        """
        print("Shard ID: {0}, Shard lenght: {1}".format(element[0], len(element[1])))
        yield element
...
class ParseXML(DoFn):
    def process(self, message_body):
        """Parse all tags and attributes from an XML and serialize them to a dict for later storage."""
...
class WriteToGCS(DoFn):
    ...
    def process(self, key_value, window=DoFn.WindowParam):
        ...
        filename = "{0}streaming_data-{1}-{2}-{3}.txt".format(self.output_path, window_start, window_end, str(shard_id))

        with io.gcsio.GcsIO().open(filename=filename, mode="w") as f:
            f.write(f"{parsed_payload}\n".encode("utf-8"))


def run(project_id, input_topic, gcs_path, window_size, num_shards, runner, region, pipeline_args=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--runner",
        help="""The beam runner to be used. For cloud Dataflow: 'DataflowRunner'. For local debugging: 'DirectRunner'. [Defaults to: 'DataflowRunner']""",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of shards to use when writing windowed elements to GCS. [Defaults to: 5]",
    )
