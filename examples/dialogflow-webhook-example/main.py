  ...
  def account_transactions_handler(self, **kwargs):
    ...
    for user in users:
      ...
      for transaction in transactions:
        checking_transactions = ','.join([
            '{} {}'.format(transaction['type'], transaction['amount']),
            checking_transactions
        ])
      transactions = user_dict['accounts']['saving']['transactions']
      for transaction in transactions:
        saving_transactions = ','.join([
            '{} {}'.format(transaction['type'], transaction['amount']),
            saving_transactions
        ])
      return json.dumps({
          'fulfillmentText':
              ' '.join([
                  message, saving_transactions, checking_transactions,
