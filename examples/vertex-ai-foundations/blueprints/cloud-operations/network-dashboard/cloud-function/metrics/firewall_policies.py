...
def get_firewal_policies_data(config, metrics_dict, firewall_policies_dict):
  ...
  '''
    ...
    try:
        ...
        print(
            "Could not determine number of tuples metric limit due to missing default value"
        )
    if current_tuples_limit < 0:
        print(
            "Could not determine number of tuples metric limit  as default value is <= 0"
        )
    ...
    for firewall_policy_key in firewall_policies_dict:
        ...
        if parent_type == "projects" and parent not in config[
                "monitored_projects"]:
            continue
        ...
        metrics.append_data_to_series_buffer(
            config,
            metrics_dict["metrics_per_firewall_policy"]
            ["firewall_policy_tuples"]["usage"]["name"],
            firewall_policy['ruleTupleCount'],
            metric_labels,
            timestamp=timestamp)
        if not current_tuples_limit == None and current_tuples_limit > 0:
            metrics.append_data_to_series_buffer(
                config,
                metrics_dict["metrics_per_firewall_policy"]
                ["firewall_policy_tuples"]["limit"]["name"],
                current_tuples_limit,
                metric_labels,
                timestamp=timestamp)
            metrics.append_data_to_series_buffer(
                config,
                metrics_dict["metrics_per_firewall_policy"]
                ["firewall_policy_tuples"]["utilization"]["name"],
                firewall_policy['ruleTupleCount'] / current_tuples_limit,
                metric_labels,
                timestamp=timestamp)

    print("Buffered number tuples per Firewall Policy")
