...
def rotate_service_account_keys_in_secret_manager(event, context):
    ...
    if (event_type == "SECRET_ROTATE"):
        ...
        current_secret_with_version = client.list_secret_versions(request={"parent": secret_name}).versions[0].name
        current_secret = client.access_secret_version(name=current_secret_with_version)
        ...
        new_key = service.projects().serviceAccounts().keys().create(
            name='projects/'+ project_id +'/serviceAccounts/' + service_account, body={}
            ).execute()
        new_service_key_json = base64.b64decode(new_key['privateKeyData'])
        client.add_secret_version(parent=secret_name, payload={'data': new_service_key_json})

        ### Delete the old service account 
        key_to_delete = "projects/{project_id}/serviceAccounts/{service_account}/keys/{key_id}".format(project_id=project_id, service_account=service_account, key_id=key_id)
        service.projects().serviceAccounts().keys().delete(
            name=key_to_delete).execute()

        ### Disable and delete the old secret version
        client.disable_secret_version(request={"name": current_secret_with_version})
        client.destroy_secret_version(request={"name": current_secret_with_version})
