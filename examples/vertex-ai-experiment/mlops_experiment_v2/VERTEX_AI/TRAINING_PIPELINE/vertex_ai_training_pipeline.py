...
def pipeline(project_id: str,location: str,training_container_uri: str,prediction_container_uri: str,
    ...
    with dsl.Condition(model_validation_op.outputs['is_model_valid'] == "TRUE"):
            create_endpoint_op = gcc_aip.EndpointCreateOp(
                                    project=project_id,
                                    display_name = vertex_ai_end_point,
                                    location=location 
                                    )
...
def compile_pipeline():
    compiler.Compiler().compile(pipeline_func=pipeline,package_path=TRAINING_PIPELINE_NAME+'.json')
...
def run_pipeline():
    job = aip.PipelineJob(
        display_name=mlops_pipeline_version,
        template_path=TRAINING_PIPELINE_NAME+'.json',
