...
/**
 * A wrapper class for general utility commands such as coping the files in GCS, creating buckets and removing buckets in GCS
 * Creating BigQuery datasets.
 */
public class Utils {
    public static boolean checkIfStringIsInList(List<String> lines, String linetoMatch){
    ...
    public static BashOutput copyFileFromLocalToGCS(String gcsPath, String localPath) throws Exception {
    ...
    public static void saveQueryResultToFile(String query, String filePath) throws Exception {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath, true))) {
            for (FieldValueList row : BQUtils.getResult(query)) {
                writer.write(row.get(0).getStringValue() + "," + row.get(1).getStringValue() + "," + row.get(2).getStringValue() + "," +
                        row.get(3).getStringValue() + "," + row.get(4).getStringValue() + "," + row.get(5).getStringValue() + "," +
                        row.get(6).getStringValue() + "," + row.get(7).getStringValue() + "," + row.get(8).getStringValue() + "," +
                        row.get(9).getStringValue() + "," + row.get(10).getStringValue() + "," + row.get(11).getStringValue() + "," +
                        row.get(12).getStringValue() + "," + row.get(13).getStringValue() + "\n");
            }
        }
    }
}
