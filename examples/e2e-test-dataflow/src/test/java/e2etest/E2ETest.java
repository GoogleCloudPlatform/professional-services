...
public class E2ETest {
    ...
    private final static Logger LOGGER = Logger.getLogger(E2ETest.class.getName());
    ...
    public void using_the_and_the_following_properties(String jarName, io.cucumber.datatable.DataTable dataTable) throws Exception {
        this.jarName =  jarName;
    ...
    public void run_dataflow_application_main_class_being_with_proper_parameters(String mainClass)  throws Exception{

        StringBuffer sb = new StringBuffer();
        String[] feildForDataflowParams = {"tempLocation","project","serviceAccount","runner","dataset","tableName","inputFilePattern"};
    ...
    public void the_Query_returns_records(String query, String count) throws Exception{
        long countData = Long.valueOf(count);
        Iterable<FieldValueList> it = BQUtils.getResult(String.format(query, myDataTable.get("dataset"),myDataTable.get("errorTableName")));
        long countFromResult = -1L;
        for (FieldValueList row : it) {
            countFromResult = row.get("count").getLongValue();
        }
    ...
    public void save_the_file_and_check_the_based_on_the_content(String query) throws Exception{
        ...
        Utils.saveQueryResultToFile(String.format(query, myDataTable.get("dataset"),myDataTable.get("tableName")),filePath);
