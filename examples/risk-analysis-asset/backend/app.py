...
app.logger.info(f"FRONTEND_SERVICE_URL : {frontend_url}")
CORS(app)
...
try:
    ...
    app.logger.info(f"successfully connected to firestore, project_id ={project_id},database_id ={database_id} ")
except Exception as e:
    app.logger.error(f"failed to connect to firestore, project_id ={project_id},database_id ={database_id} ")
...
## uncomment for CORS support, Cloud Run doesnt support CORS if unauthenticated requests are disallowed
...
def create_application():
    ...
    try:
        ...
        print(jsonify(app_ref))
        return jsonify({"message": "Application created successfully", "appId": app_ref.id}), 201
...
def list_application():
    ...
    try:
        ...
        app_list = []
        for app in apps:
            cuj_data = app.to_dict()
            cuj_data['id'] = app.id  # Include the document ID in the response
            app_list.append(cuj_data)
...
def create_critical_user_journey():
    ...
    try:
        ...
        if cuj_ref.id:
            ...
            if cuj_ref.id not in current_cuj_ids:  # Check if the CUJ ID is already present
                current_cuj_ids.append(cuj_ref.id)
                app_ref.update({
                    'CUJIds': current_cuj_ids
                })

        return jsonify({"message": "Critical User Journey created successfully", "cujId": cuj_ref.id}), 201
...
def get_cuj_details(cuj_id):
    ...
    try:
        ...
        return jsonify({"error": f"Error fetching Critical User Journey details: {e}"}), 500
...
def update_cuj(cuj_id):
    ...
    try:
        ...
        try:
            ...
            app.logger.error(f"failed to get request json {e}")
        updated_keys = list(data.keys())
        risk_id = str(data.get('RiskIDs'))
        # Reference to the specific CUJ document
        cuj_ref = db.collection('Critical User Journeys').document(cuj_id)
        ...
        #current_risk_ids = cuj_doc.to_dict().get('RiskIDs', [])  # Get existing RiskIDs or an empty list if not present
        # Combine existing RiskIDs with new ones (avoiding duplicates)
        field_names = list(cuj_doc.to_dict().keys())
        update_data = {}
...
def remove_risk_from_cuj(cuj_id, risk_id, bad_mins):
    ...
    try:
        ...
        return jsonify({"message": f"Risk {risk_id} removed from Critical User Journey successfully"}), 200
...
def add_risk_theme():
    ...
    try:
        ...
        return jsonify({"message": "Risk theme added successfully", "riskThemeId": theme_ref.id}), 201
...
def save_risk():
    ...
    try:
        ...
        # Data validation (add more as needed)
        if not data.get('riskName'): # or not data.get('riskTheme') or not data.get('journeyName'):  # journeyName is now an array
            return jsonify({"error": "Missing required fields"}), 400
        ...
        return jsonify({"message": "Risk saved successfully", "riskId": risk_ref.id}), 201
...
def update_risk(risk_id):
    ...
    try:
        ...
        update_data = {field: data[field] for field in data if field in risk_doc.to_dict()}
            ...
            for cuj_id in cuj_ids:
                ...
                if cuj_doc.exists:
                    for i in range(3):
                        ...
                        old_accepted_bad_mins = cuj_doc.to_dict().get(accepted_bad_mins_field, [])
                        if risk_id in accepted_risk_ids:
                ...
                if new_accepted_bad_mins:
                    for j in range(len(new_accepted_bad_mins)):
                        if new_accepted_bad_mins[j] is not None:  # Check if value exists at index i
                            cuj_data[f'accepted{j}'] = new_accepted_bad_mins[j]
                    cuj_ref.update(cuj_data)
...
def get_risks_for_cuj(cuj_id):
    ...
    try:
        ...
        risk_docs = risks_ref.where('__name__', 'in', risk_ids).stream()  # Batch query for efficiency
                
        # Fetch all risk themes in one go
        themes_ref = db.collection('Risk Themes')
        theme_docs = themes_ref.stream()
        theme_map = {theme_doc.id: theme_doc.to_dict()['ThemeName'] for theme_doc in theme_docs}

        # Fetch all CUJs in one go
        list_cujs_ref = db.collection('Critical User Journeys')
        list_cujs_docs = list_cujs_ref.stream()
        list_cujs_map = {list_cujs_doc.id: list_cujs_doc.to_dict() for list_cujs_doc in list_cujs_docs}
        ...
        for risk_doc in risk_docs:
            ...
            risk_data['riskThemes'] = [{'riskThemeID': theme_id, 'ThemeName': theme_map.get(theme_id)} for theme_id in risk_theme_ids]
            del risk_data['riskThemeID']

            # Replace journeyID with journey details
            journey_id = risk_data.get('journeyID', [])  
            risk_data['journeyDetails'] = [{**list_cujs_map.get(cuj_id), 'JourneyID': cuj_id} for cuj_id in journey_id]
