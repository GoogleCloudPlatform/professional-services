...
/** Class represents a task specifically focusing on redis list operations. */
public class ListOperationsTask implements RedisTask {
  private static final Logger LOG = LoggerFactory.getLogger(ListOperationsTask.class);
  ...
  private byte[][] getitems() {
    int totalItems = 10 + random.nextInt(5);
    byte[][] items = new byte[totalItems][];
    IntStream.range(0, totalItems).forEach(i -> items[i] = this.payloadGenerator.getPayloadBytes());
    return items;
  }

  /** Core execution logic of the Redis List operations (lpush and rpop). */
  @Override
  public void run() {
    String key = null;
    for (int i = 1; i <= 100; i++) {
      if (counter < 10 || Math.random() < writeRatio) { // Write the value
        ...
        jedisClient.lpush(key.getBytes(StandardCharsets.UTF_8), getitems());
        counter++;
      } else {
        key = String.format("%s:%s", this.keyPrefix, MathUtils.getRandomLong(0, counter));
        long startTime = System.nanoTime();
        if (jedisClient.rpop(key.getBytes(StandardCharsets.UTF_8)) != null) {
          hits++;
        } else {
          if (!jedisClient.exists(key)) misses++;
        }
      }
    }
  ...
  public void cleanUp() {
    ...
    LOG.info(
        "Total List Writes: {}, List reads: {}, percent of hits:{} and percent of misses:{} in client:{}",
        counter,
        totalReads,
