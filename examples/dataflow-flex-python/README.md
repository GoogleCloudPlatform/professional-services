...
│   │   ├── customer_orders_model.py --> Data Model for input data, generated by xsdata
...
└── sample-data
    ├── customer-orders.xml --> Sampe Customer Order Data
    └── customer_orders.xsd --> Sampe Customer Order Data model
```
...
#### Upload Sample Data
```
gcloud storage cp ./sample-data/customer-orders.xml gs://$INPUT_BUCKET_NAME/data/
```
...
```
gcloud dataflow flex-template run xml-to-bq-sample-pipeline-3 \
...
Data Model has been borrowed from https://learn.microsoft.com/en-in/dotnet/standard/linq/sample-xml-file-customers-orders-namespace#customersordersinnamespacexml
