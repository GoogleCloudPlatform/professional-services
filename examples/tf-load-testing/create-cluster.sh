...
function create_cluster() {
	...
	local MASTER_CIDR=$(gcloud container clusters describe ${CLUSTER_NAME} --project=${GCP_PROJECT} --zone=${CLUSTER_ZONE} --format="value(privateClusterConfig.masterIpv4CidrBlock)" 2>> /dev/null)
