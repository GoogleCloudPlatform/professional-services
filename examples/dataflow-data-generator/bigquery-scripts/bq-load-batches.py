...
def get_bq_client_json_load_config(project, max_bad_records):
    """
    Helper function to create a BigQuery Client and a load job
    configuration for JSON files.

    Args:
        project: (str) GCP project_id.
        max_bad_records: (int) Maximum allowed bad records per 
            load job to not fail.
    Return:
        bq_cli: (bigquery.Client)
        job_config: (bigquery.job.LoadConfig) for newline 
            delimited json files, with write append disposition.        
    """
    # Prepare to create the bq load jobs.
    bq_cli = bq.Client(project=project)
...
def parse_gsutil_long_output_file(filename):
    ...
    batches = []
    for i in xrange(int(df['batch_num'].max())):
...
def main(argv=None):
    ...
    parser.add_argument('--partition_column', dest='partition_column',
                        required=False, default=None)
    parser.add_argument('--max_bad_records', dest='max_bad_records',
                        required=False, default=1)
    known_args, _ = parser.parse_known_args(argv)
    
    bq_cli, job_config = get_bq_client_json_load_config(
        project=known_args.project, 
        max_bad_records=int(known_args.max_bad_records)
    )
