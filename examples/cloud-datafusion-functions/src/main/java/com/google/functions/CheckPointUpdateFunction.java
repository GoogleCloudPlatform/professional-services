...
public class CheckPointUpdateFunction {
	...
	public void execute(String serviceAccountFilePath, String project, String collectionName, String documentName, String incrPullTableDataset,  
			String incrPullLogTableName, String logTableCheckpointColumn) throws Exception {
		CheckpointDAO checkpointDAO = getCheckpointDAO(serviceAccountFilePath, project);
		String maxTimestampValue = getMaxTimestampValueFromLogTable(serviceAccountFilePath, project, 
			incrPullTableDataset, incrPullLogTableName, logTableCheckpointColumn);
		if (maxTimestampValue == null) {
			maxTimestampValue = getMaxValueFromLogTable(serviceAccountFilePath, project, incrPullTableDataset, incrPullLogTableName, logTableCheckpointColumn);
		}
	...
	private String getMaxTimestampValueFromLogTable(String serviceAccountFilePath, String projectId, 
		    String incrPullTableDataset, String incrPullLogTableName, String logTableCheckpointColumn) {
		String sql = null;
	        ...
	        for (FieldValueList row : result.iterateAll()) {
	        	if (row.get("CHECKPOINT_VALUE").getValue() != null) {
		        	...
		        	if (latestCheckpointValue != null) {
		        		latestCheckpointValue = latestCheckpointValue.substring(0, latestCheckpointValue.indexOf("+"));
		        	}
	        	}
	        }
	        LOG.info("latestCheckpointValue == " + latestCheckpointValue);
		} catch(Exception error) {
			LOG.info("checkpoint column logTableCheckpointColumn may not be a timestamp field == " + logTableCheckpointColumn);
	...
	private String getMaxValueFromLogTable(String serviceAccountFilePath, String projectId, 
		    String incrPullTableDataset, String incrPullLogTableName, String logTableCheckpointColumn) throws Exception {
		String sql = null;
	...
	private BigQuery getBigQuery(@Nullable String serviceAccountFilePath, @Nullable String project) throws Exception {
