...
import re
import unittest
from sideinput_refresh import main, transforms,coders
...
class TestSubroutines(unittest.TestCase):
    def test_UTF8JsonCoder_valid_message(self):
        ...
        self.assertEqual(utf8_jsoncoder.encode(message), utfbytes, "UTF8JsonCoder encoding failed")

        #Check decode functionality
        utfbytes = json.dumps(message).encode()
        self.assertEqual(utf8_jsoncoder.decode(utfbytes), message, "UTF8JsonCoder decoding failed")
    ...
    def test_UTF8JsonCoder_invalid_message(self):
        ...
        with self.assertLogs(logger=None, level='INFO') as log:
            ...
            self.assertIsNotNone(re.match(pattern, log.output[0]),"Invalid utf8 input is not raising decode error")
...
class TestTransforms(unittest.TestCase):
    ...
    def test_transform_kv_of(self):
        ...
        key_vaue_pair = transforms.kv_of(event, "product_name",
                                                "bonuspoints")
