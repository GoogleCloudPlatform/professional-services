...
class FakeRowGen(beam.DoFn):
    ...
    """
    def __init__(self, data_gen, key_set=None):
        """
    ...
    def trunc_norm_trendify(self, loc, var_scale=0.01):
        """
...
class _RoundRobinKeyFn(beam.DoFn):
    """
    This is a DoFn to shard data evenly among self.count keys.
    Attributes:
        count (int): Number of keys to shard the data across.
        counter (int): The key that this element will be assigned to.
    """
    ...
    def start_bundle(self):
        # Start the counter at a random location.
        self.counter = random.randint(0, self.count - 1)

    def process(self, element):
        # This rotates counter through the range [0, self.count-1]
        self.counter += 1
        if self.counter >= self.count:
            self.counter -= self.count

        # We yield k,v pairs where the values are evenly distributed across the
        # keys.
...
class LimitBundles(beam.PTransform):
    """
    The WriteToBigQuery transform requires <= 10000 bundles. This PTrasnform was
    created to evenly shard elements recieved from the prior PCollection
    across self.count bundles.
    """
    ...
    def expand(self, pcol):
        return(pcol
            | beam.ParDo(_RoundRobinKeyFn(self.count))  # Evenly shard k,v pairs.
            | beam.GroupByKey()  # Bundle based on the keys.
            | beam.FlatMap(lambda kv: kv[1]))  # Drop meaningless keys.
...
def parse_data_generator_args(argv):
    ...
    parser.add_argument('--write_disposition', dest='write_disp',
                        required=False,
                        help='WriteDisposition for BigQuery. Options are '
                             'WRITE_APPEND, WRITE_EMPTY WRITE_TRUNCATE.',
                        default='WRITE_APPEND')

    parser.add_argument('--fact_table', dest='fact_table',
                        help='Side input table to select key set from .',
                        default=None)
    ...
    # parser.add_argument('--injection_volume', dest='injection_volume',
    #                     required=False,
    #                     help='number of messages to write at a time',
    #                     default='100')
    #
    # parser.add_argument('--injection_interval', dest='injection_interval',
    #                     required=False,
    #                     help='time between writing to pubsub in seconds',
    #                     default='10')
    parser.add_argument('--pub_frequency', dest='pub_frequency',
                        required=False,
                        help='publishing rate in messages per second',
                        default='1000')

    parser.add_argument('--total_volume', dest='total_volume',
                        required=False,
                        help='Total Number of messages to write',
                        default='100000')
