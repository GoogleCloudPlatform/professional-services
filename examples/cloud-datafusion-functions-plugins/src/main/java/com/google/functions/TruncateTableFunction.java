...
public class TruncateTableFunction {
    ...
    public static void truncateTable(String keyPath, String projectId, String dataset, String tableName ) {
        try {
            ...
            QueryJobConfiguration queryConfig = QueryJobConfiguration.newBuilder(query).setUseLegacySql(false)
            ...
            queryConfig = QueryJobConfiguration.newBuilder(query).setUseLegacySql(false)
                    .build();
    ...
    public static void updateTableField(String keyPath, String projectId, String dataset, String tableName, String field, String value, String where) {
        BigQuery bigquery = Utils.createBigQueryClient(keyPath);

        String query = String.format("UPDATE `%s`.`%s`.`%s` SET %s = %s WHERE %s",
    ...
    private static String getTableValue(String keyPath, String projectId, String dataset, String tableName, String field, String where) throws InterruptedException, InvocationTargetException {
        String result = "0";

        BigQuery bigquery = Utils.createBigQueryClient(keyPath);

        String query = String.format("SELECT %s FROM `%s`.`%s`.`%s` WHERE %s",
                field, projectId, dataset, tableName, where);
    ...
    private static TimePartitioning getTablePartitioning (Table table)
    {
        ...
        if (table.<StandardTableDefinition>getDefinition().getTimePartitioning() != null) {
            partitionField = table.<StandardTableDefinition>getDefinition().getTimePartitioning().getField();
            partitionType = table.<StandardTableDefinition>getDefinition().getTimePartitioning().getType();
