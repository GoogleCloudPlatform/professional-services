...
# --- Cleanup Function ---
cleanup() {
    printf '\nShutting down backend API server (PID: %s)...\n' "$API_PID"
...
# --- Start Frontend UI ---
# API_PORT is already exported above, so no need to prefix it here.
echo "Starting Streamlit frontend on port $STREAMLIT_PORT..."
streamlit run app_ui.py --server.port "$STREAMLIT_PORT" --server.headless true
