...
public class CustomRoleAnalyzerHelper {
    ...
    /**
     * Initialize the analysis by fetching all the predefined roles, org level
     * permissions, maping permissions to role and removing testing permissions from
     * predefined roles.
     * 
     * @param orgId
     */
    ...
    public void initilize(String orgId, String resultFormat) {
        ...
        String resourcePath = GenericConstants.CLOUD_RESOURCE_MANAGER_API + GenericConstants.SEPARATOR
                + GenericConstants.ORGANIZATIONS + GenericConstants.SEPARATOR + orgId;

        try {
            predefinedRoleSet = iamClient.fetchPredefinedRoles();
            allPermissions = permissionUtil
                    .fetchAllPermissions(resourcePath);
        ...
        predefinedRolesWithSupportedPermissions = roleUtil
                .removeTestingPermission(predefinedRolesToPermissionsMap, testingPermissions);
    }
    ...
    private void printResult(String parent, Map<String, Set<String>> matchingRoles, Role customRole) {
        ...
        logger.atInfo().log("Total number of maching roles found for " + customRole.getName() + ":" + keySet.size());
    ...
    private void writeResultToFile(CustomRoleAnalyzerResult analyzerResult) {
        ...
        try {
            if (resultFormat.equals(GenericConstants.DEFAULT_FORMAT)) {
                String resultFileName = GenericConstants.RESULT_FILENAME + "." + GenericConstants.DEFAULT_FORMAT;
                if (resultsFile == null) {
                ...
                resultsFile.append(analyzerResult.getCustomRole() + ", " + analyzerResult.getParent()
                        + ", " + predefinedRoleLStringBuffer + ", "
                        + additionPermissionsStringBuffer + "," + analyzerResult.isExactMatch + "\n");

            } else {
                String resultFileName = GenericConstants.RESULT_FILENAME + "." + GenericConstants.JSON_FORMAT;
    ...
    private void processCustomRoles(String parent, boolean isOrgLevel) {
        ...
        while (iterator.hasNext()) {
            ...
            Map<String, Set<String>> possibleMatchingRoles = roleUtil.findPosibleMatchingRoles(customRolePermissionsSet,
                    predefinedRolesWithSupportedPermissions);

            Map<String, Set<String>> matchingRoles = roleUtil.findMachingPredefindedRoles(role, possibleMatchingRoles);
    ...
    public void processProjectLevelCustomRoles(String org_id) {
        ...
        String resourcePath = GenericConstants.CLOUD_RESOURCE_MANAGER_API + GenericConstants.SEPARATOR + parentProject;
        ...
        for (Iterator<Map.Entry<Folder, Set<Project>>> it = folderAndProjectsMap.entrySet().iterator(); it.hasNext();) {
            ...
            if (projects.size() > 0) {
                ...
                while (iterator.hasNext()) {
                     ...
                     * Need to add such a permissions in the list of permissions which we need
                    ...
                    if (allPermissionsProjectLevel == null) {
                        ...
                        try {
                            permissionsProjectLevel = permissionUtil
                                    .fetchAllPermissions(resourcePath);

                        } catch (IOException | GeneralSecurityException e) {
                            logger.atSevere().withCause(e).log("Exception while feching permissions: " + resourcePath);
                        }
                        allPermissionsProjectLevel = permissionUtil
                                .convertPermissionsToStringSet(permissionsProjectLevel);
                        allOrgPermissionsStringSet = permissionUtil
                                .convertPermissionsToStringSet(allPermissions);
                        Set<String> differenceSet = Sets.difference(allOrgPermissionsStringSet,
