...
public class PermissionUtil {
     ...
     */
    public Set<Permission> fetchAllPermissions(String resouceName) throws IOException, GeneralSecurityException {
        ...
        Iam.Permissions.QueryTestablePermissions request = iamService.permissions()
                .queryTestablePermissions(requestBody);
        ...
        logger.atInfo().log("Total number of permissions at " + resouceName + ":" + permissionSet.size());
        return permissionSet;
    }

    /**
     * Convert permissions to String set so as to find differences and
     * intersections.
     * 
     * @param permissionSet
    ...
    /**
     * Filter the pemissions which are in testing mode but also suppoted on custom
     * role.
     * permission.getCustomRolesSupportLevel() is only set on TESTING or
     * NOT_SUPPORTED permissions otherwise it is null.
     * 
    ...
    public Set<String> findTestingModePermissions(Set<Permission> allPermissions) {
        ...
        while (iterator.hasNext()) {
            ...
            if (permission.getCustomRolesSupportLevel() != null) { // "customRolesSupportLevel":"TESTING" or
        ...
        logger.atInfo().log("Total number of TESTING or NOT_SUPPORTED permissions: " + filteredPermissions.size());
