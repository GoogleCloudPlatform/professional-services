...
public class RoleUtil {
    ...
    /**
     * Remove TESTING or UNSUPPORTED permissions from the predefined roles.
     * This is required as predefined roles always has TESTING stage permissions and
     * those are not recommended for custom roles.
     * Assuming custom roles does not have TESTING stage permissions.
     * 
    ...
    public Map<String, Set<String>> removeTestingPermission(
        ...
        for (Iterator<Map.Entry<String, Set<String>>> it = tempRp.entrySet().iterator(); it.hasNext();) {
            ...
            if (intersection.size() > 0) {
                logger.atFine().log("Number of TESTING permissions removed from " + role + ":" + intersection.size());
     ...
     */
    public Map<String, Set<String>> findPosibleMatchingRoles(Set<String> customRolePermissions,
            Map<String, Set<String>> predefinedRolesPermissions) {
     ...
     * @return Map of predefinded role and its permissions.
     */
    public Map<String, Set<String>> findMachingPredefindedRoles(Role customRole,
            Map<String, Set<String>> predefinedRolesMap) {

        logger.atInfo().log("Finding exact maching predefined roles for custom role: " + customRole.getName());
        Map<String, Set<String>> processedPredefinedRolesMap = new HashMap<String, Set<String>>(predefinedRolesMap);
        ...
        do {
            int countMax = 0;
            for (Iterator<Map.Entry<String, Set<String>>> it = processedPredefinedRolesMap.entrySet().iterator(); it
                    .hasNext();) {
                Map.Entry<String, Set<String>> entry = it.next();
            ...
            entryRemoved = entryRemoved
                    + checkDuplicatesRoles(processedPredefinedRolesMap, processedPredefinedRolesMap.get(index), index);
    ...
    /**
     * Check if there are any duplicate predefined roles in possible maching roles and remove those from the map.
     * 
     * @param predefinedRolesMap
    ...
    private int checkDuplicatesRoles(Map<String, Set<String>> predefinedRolesMap, Set<String> maxPermissions,
            String index) {

        logger.atFine().log("Checking for duplicate roles");
        int entryRemoved = 0;

        for (Iterator<Map.Entry<String, Set<String>>> it = predefinedRolesMap.entrySet().iterator(); it.hasNext();) {
            Map.Entry<String, Set<String>> entry = it.next();
